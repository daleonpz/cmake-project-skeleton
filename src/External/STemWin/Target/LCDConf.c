 ...
 * @{
 */
static void DMA2D_CopyBuffer(U32 LayerIndex, void* pSrc, void* pDst, U32 xSize, U32 ySize,
							 U32 OffLineSrc, U32 OffLineDst);
static void DMA2D_FillBuffer(U32 LayerIndex, void* pDst, U32 xSize, U32 ySize, U32 OffLine,
							 U32 ColorIndex);
...
static void CUSTOM_DrawBitmap32bpp(int LayerIndex, int x, int y, U8 const* p, int xSize, int ySize,
								   int BytesPerLine);
...
void HAL_LTDC_LineEventCallback(LTDC_HandleTypeDef* hltdc)
{
	...
	for(layer = 0; layer < GUI_NUM_LAYERS; layer++)
	{
		if(layer_prop[layer].pending_buffer >= 0)
		{
			...
			Addr = layer_prop[layer].address + layer_prop[layer].xSize * layer_prop[layer].ySize *
	...
	for(i = 0; i < GUI_NUM_LAYERS; i++)
	{
		...
		if(LCD_LL_GetPixelformat(i) == LTDC_PIXEL_FORMAT_ARGB8888)
		{
			LCD_SetDevFunc(
				...
				(void (*)(
					void))CUSTOM_DrawBitmap32bpp); /* Set up drawing routine for 32bpp bitmap using
													  DMA2D. Makes only sense with ARGB8888 */
		}
	}
}
...
int LCD_X_DisplayDriver(unsigned LayerIndex, unsigned Cmd, void* pData)
{
	...
	switch(Cmd)
	{
		...
		case LCD_X_SETORG:
			addr = layer_prop[LayerIndex].address + ((LCD_X_SETORG_INFO*)pData)->yPos *
		...
		case LCD_X_SETLUTENTRY:
			HAL_LTDC_ConfigCLUT(&hltdc, (uint32_t*)&(((LCD_X_SETLUTENTRY_INFO*)pData)->Color), 1,
								LayerIndex);
			break;
...
static void CUSTOM_CopyBuffer(int LayerIndex, int IndexSrc, int IndexDst)
{
	...
	DMA2D_CopyBuffer(LayerIndex, (void*)AddrSrc, (void*)AddrDst, layer_prop[LayerIndex].xSize,
					 layer_prop[LayerIndex].ySize, 0, 0);
...
static void CUSTOM_CopyRect(int LayerIndex, int x0, int y0, int x1, int y1, int xSize, int ySize)
{
	...
	AddrDst = layer_prop[LayerIndex].address +
			  (y1 * layer_prop[LayerIndex].xSize + x1) * layer_prop[LayerIndex].BytesPerPixel;
	DMA2D_CopyBuffer(LayerIndex, (void*)AddrSrc, (void*)AddrDst, xSize, ySize,
					 layer_prop[LayerIndex].xSize - xSize, layer_prop[LayerIndex].xSize - xSize);
}
								   ...
								   int BytesPerLine)
{
	...
	BufferSize = GetBufferSize(LayerIndex);
	AddrDst = layer_prop[LayerIndex].address + BufferSize * layer_prop[LayerIndex].buffer_index +
			  (y * layer_prop[LayerIndex].xSize + x) * layer_prop[LayerIndex].BytesPerPixel;
