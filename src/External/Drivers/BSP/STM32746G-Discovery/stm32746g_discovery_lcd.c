...
/** @defgroup STM32746G_DISCOVERY_LCD_Private_TypesDefinitions STM32746G_DISCOVERY_LCD Private Types
 * Definitions
 * @{
 */
/**
...
/** @defgroup STM32746G_DISCOVERY_LCD_Private_FunctionPrototypes STM32746G_DISCOVERY_LCD Private
 * Function Prototypes
 * @{
 */
static void DrawChar(uint16_t Xpos, uint16_t Ypos, const uint8_t* c);
static void FillTriangle(uint16_t x1, uint16_t x2, uint16_t x3, uint16_t y1, uint16_t y2,
						 uint16_t y3);
 ...
 * @retval None
 */
void BSP_LCD_SetLayerWindow(uint16_t LayerIndex, uint16_t Xpos, uint16_t Ypos, uint16_t Width,
							uint16_t Height)
{
 ...
 * @retval None
 */
void BSP_LCD_SetLayerWindow_NoReload(uint16_t LayerIndex, uint16_t Xpos, uint16_t Ypos,
...
{
	/* Clear the LCD */
	LL_FillBuffer(ActiveLayer, (uint32_t*)(hLtdcHandler.LayerCfg[ActiveLayer].FBStartAdress),
...
void BSP_LCD_DisplayChar(uint16_t Xpos, uint16_t Ypos, uint8_t Ascii)
{
	DrawChar(
		Xpos, Ypos,
		&DrawProp[ActiveLayer].pFont->table[(Ascii - ' ') * DrawProp[ActiveLayer].pFont->Height *
...
void BSP_LCD_DisplayStringAt(uint16_t Xpos, uint16_t Ypos, uint8_t* Text, Text_AlignModeTypdef Mode)
{
	...
	while((*Text != 0) & (((BSP_LCD_GetXSize() - (i * DrawProp[ActiveLayer].pFont->Width)) &
...
void BSP_LCD_DrawVLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
	...
	/* Write line */
	LL_FillBuffer(ActiveLayer, (uint32_t*)Xaddress, 1, Length, (BSP_LCD_GetXSize() - 1),
...
void BSP_LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	int16_t deltax = 0, deltay = 0, x = 0, y = 0, xinc1 = 0, xinc2 = 0, yinc1 = 0, yinc2 = 0,
			den = 0, num = 0, num_add = 0, num_pixels = 0, curpixel = 0;
...
void BSP_LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGB_Code)
{
	...
	{ /* RGB565 format */
		*(__IO uint16_t*)(hLtdcHandler.LayerCfg[ActiveLayer].FBStartAdress +
						  (2 * (Ypos * BSP_LCD_GetXSize() + Xpos))) = (uint16_t)RGB_Code;
	}
	else
	{ /* ARGB8888 format */
...
/**
 * @brief  Draws a bitmap picture loaded in the internal Flash in ARGB888 format (32 bits per
 * pixel).
...
void BSP_LCD_FillRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
	...
	/* Fill the rectangle */
	LL_FillBuffer(ActiveLayer, (uint32_t*)x_address, Width, Height, (BSP_LCD_GetXSize() - Width),
				  DrawProp[ActiveLayer].TextColor);
}
...
{
	int16_t X = 0, Y = 0, X2 = 0, Y2 = 0, X_center = 0, Y_center = 0, X_first = 0, Y_first = 0,
...
void BSP_LCD_DisplayOn(void)
{
	...
	HAL_GPIO_WritePin(LCD_BL_CTRL_GPIO_PORT, LCD_BL_CTRL_PIN,
					  GPIO_PIN_SET); /* Assert LCD_BL_CTRL pin */
}
...
void BSP_LCD_DisplayOff(void)
{
	/* Display Off */
	__HAL_LTDC_DISABLE(&hLtdcHandler);
	HAL_GPIO_WritePin(LCD_DISP_GPIO_PORT, LCD_DISP_PIN,
					  GPIO_PIN_RESET); /* De-assert LCD_DISP pin */
	HAL_GPIO_WritePin(LCD_BL_CTRL_GPIO_PORT, LCD_BL_CTRL_PIN,
					  GPIO_PIN_RESET); /* De-assert LCD_BL_CTRL pin */
}
...
__weak void BSP_LCD_MspDeInit(LTDC_HandleTypeDef* hltdc, void* Params)
{
	...
	/* GPIOI deactivation */
	gpio_init_structure.Pin = GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_12 | GPIO_PIN_13 |
