...
HAL_StatusTypeDef HAL_ADC_Start(ADC_HandleTypeDef* hadc)
{
	...
	if(HAL_IS_BIT_SET(hadc->Instance->CR2, ADC_CR2_ADON))
	{
		...
		else
		{
			...
			{
				/* if instance of handle correspond to ADC3 and no external trigger present enable
				 * software conversion of regular channels */
...
HAL_StatusTypeDef HAL_ADC_ConfigChannel(ADC_HandleTypeDef* hadc, ADC_ChannelConfTypeDef* sConfig)
{
	...
	/* if ADC1 Channel_18 or Channel_17 is selected enable TSVREFE Channel(Temperature sensor and
	 * VREFINT) */
