/**
  ...
  ******************************************************************************
  ...
  ==============================================================================
	[..]
	 (#) Initialize the JPEG peripheral using HAL_JPEG_Init : No initialization parameters are
  required. Only the call to HAL_JPEG_Init is necessary to initialize the JPEG peripheral.
	 ...
	 (#) Note that for decoding operation the JPEG peripheral output data are organized in
		 ...
		 It is up to the application to transform these YCbCr blocks to RGB data that can be
  display.
  ...
  HAL_JPEG_Resume. If the application has ended feeding the HAL JPEG with input data (no more input
  data), the application Should call the function HAL_JPEG_ConfigInputBuffer (within the callback
  HAL_JPEG_GetDataCallback) with the parameter InDataLength set to zero.
		 ...
		 (++) The mechanism of HAL_JPEG_ConfigInputBuffer/HAL_JPEG_Pause/HAL_JPEG_Resume allows
			  ...
			  If the new input data chunk is not available (because data should be read from an
  input file for example) the application can pause the JPEG input (using function HAL_JPEG_Pause)
			  Once the new input data chunk is available ( read from a file for example), the
  application can call the function HAL_JPEG_ConfigInputBuffer to provide the HAL with the new chunk
		 ...
		 (++) The application can call functions HAL_JPEG_ConfigInputBuffer then HAL_JPEG_Resume.
			  any time (outside the HAL_JPEG_GetDataCallback)  Once the new input chunk data
  available. However, to keep data coherency, the function HAL_JPEG_Pause must be imperatively
  called (if necessary) within the callback HAL_JPEG_GetDataCallback, i.e when the HAL JPEG has
  ended Transferring the previous chunk buffer to the JPEG peripheral.
	 ...
	 (#) Callback HAL_JPEG_DataReadyCallback is asserted when the HAL JPEG driver
		 ...
		 (++) This CallBack should be implemented in the application side. It should
			  ...
			  call the function HAL_JPEG_Pause with parameter XferSelection set to
  JPEG_PAUSE_RESUME_OUTPUT to inform the JPEG HAL driver that it shall pause output data. Once the
  application is ready to receive the new data chunk (output buffer location free or available) it
  ...
  resume outputting data in the given output buffer.
		 ...
		 (++) The mechanism of HAL_JPEG_ConfigOutputBuffer/HAL_JPEG_Pause/HAL_JPEG_Resume allows
			  ...
			  these received data (YCbCr to RGB conversion in case of decoding or data storage in
  case of encoding).

		 (++) The application can call  functions HAL_JPEG_ ConfigOutputBuffer then HAL_JPEG_Resume.
			  any time (outside the HAL_JPEG_DataReadyCallback) Once the output data buffer is free
  to use. However, to keep data coherency, the function HAL_JPEG_Pause must be imperatively called
			  (if necessary) within the callback HAL_JPEG_ DataReadyCallback, i.e when the HAL JPEG
  has ended Transferring the previous chunk buffer from the JPEG peripheral to the application.
	 ...
	 (#) By default the HAL JPEG driver uses the default quantization tables
		 ...
		 User can change these default tables if necessary using the function
  HAL_JPEG_SetUserQuantTables Note that for decoding the quantization tables are automatically
  extracted from the JPEG header.
	 ...
	 *** JPEG HAL driver macros list ***
	 ...
	 [..]
	  ...
	  (+) __HAL_JPEG_GET_IT_SOURCE      : returns the state of the specified JPEG Interrupt (Enabled
  or disabled).
 ...
 */
		...
		#define JPEG_CONTEXT_ENCODE                               \
			((uint32_t)0x00000001) /* JPEG context : operation is \
									  encoding*/
		#define JPEG_CONTEXT_DECODE                               \
			((uint32_t)0x00000002) /* JPEG context : operation is \
									  decoding*/
		#define JPEG_CONTEXT_OPERATION_MASK \
			((uint32_t)0x00000003) /* JPEG context : operation Mask */

		#define JPEG_CONTEXT_POLLING \
			((uint32_t)0x00000004) /* JPEG context : Transfer use Polling */
		...
		#define JPEG_CONTEXT_CONF_ENCODING \
			((uint32_t)0x00000100) /* JPEG context : encoding config done */
		...
		#define JPEG_CONTEXT_CUSTOM_TABLES \
			((uint32_t)0x00004000) /* JPEG context : Use custom quantization tables */
...
/*
 ...
 This implementation of Huffman table structure is compliant with ISO/IEC 10918-1 standard , Annex C
 Huffman Table specification
 */
typedef struct
{
	/* These two fields directly represent the contents of a JPEG DHT marker */
	uint8_t Bits[16]; /*!< bits[k] = # of symbols with codes of length k bits, this parameter
						 corresponds to BITS list in the Annex C */

	uint8_t HuffVal[162]; /*!< The symbols, in order of incremented code length, this parameter
							 corresponds to HUFFVAL list in the Annex C */
...
typedef struct
{
	...
	uint8_t HuffVal[12]; /*!< The symbols, in order of incremented code length, this parameter
							corresponds to HUFFVAL list in the Annex C */
...
typedef struct
{
	...
	uint32_t HuffmanCode[JPEG_DC_HUFF_TABLE_SIZE]; /*!< HuffmanCode */
		 ...
		 * @{
		 */
		#define JPEG_ENABLE_DMA(__HANDLE__, __DMA__) \
			((__HANDLE__)->Instance->CR |= ((__DMA__) & JPEG_DMA_MASK))
		/*note  : To disable a DMA request we must use MODIFY_REG macro to avoid writing "1" to the
		   FIFO flush bits located in the same DMA request enable register (CR register). */
		#define JPEG_DISABLE_DMA(__HANDLE__, __DMA__) \
			MODIFY_REG((__HANDLE__)->Instance->CR, ((__DMA__) & JPEG_DMA_MASK), 0UL)
/**
 * @}
 */
...
static HAL_StatusTypeDef JPEG_Bits_To_SizeCodes(uint8_t* Bits, uint8_t* Huffsize,
												uint32_t* Huffcode, uint32_t* LastK);
static HAL_StatusTypeDef
	JPEG_DCHuff_BitsVals_To_SizeCodes(JPEG_DCHuffTableTypeDef* DC_BitsValsTable,
									  JPEG_DC_HuffCodeTableTypeDef* DC_SizeCodesTable);
static HAL_StatusTypeDef
	JPEG_ACHuff_BitsVals_To_SizeCodes(JPEG_ACHuffTableTypeDef* AC_BitsValsTable,
...
{
	/* These are the sample quantization tables given in JPEG spec ISO/IEC 10918-1 standard ,
	 * section K.1. */
	...
	if(hjpeg->State == HAL_JPEG_STATE_RESET)
	{
		...
		hjpeg->EncodeCpltCallback =
			HAL_JPEG_EncodeCpltCallback; /* Legacy weak EncodeCpltCallback */
		hjpeg->DecodeCpltCallback =
			HAL_JPEG_DecodeCpltCallback; /* Legacy weak DecodeCpltCallback */
 ...
 *         Info Ready JPEG Callback is redirected to the weak HAL_JPEG_InfoReadyCallback()
 * predefined callback
 * @param  hjpeg JPEG handle
 * @retval HAL status
 */
 ...
 *         Get Data JPEG Callback is redirected to the weak HAL_JPEG_GetDataCallback() predefined
 * callback
 * @param  hjpeg JPEG handle
 * @retval HAL status
 */
 ...
 *         Get Data Ready Callback is redirected to the weak HAL_JPEG_DataReadyCallback() predefined
 * callback
 * @param  hjpeg JPEG handle
 * @retval HAL status
 */
...
@verbatim
  ...
  ==============================================================================
	[..]  This section provides functions allowing to:
	  ...
	  (+) HAL_JPEG_GetInfo() :  Extract the image configuration from the JPEG header during the
decoding
	  ...
	  (+) HAL_JPEG_SetUserQuantTables : Modify the default Quantization tables used for JPEG
encoding.
	...
	else
	{
		...
		if(hjpeg->State == HAL_JPEG_STATE_READY)
		{
			...
			if(hjpeg->Conf.ColorSpace == JPEG_GRAYSCALE_COLORSPACE)
			{
				...
				error =
					JPEG_Set_Quantization_Mem(hjpeg, hjpeg->QuantTable0, (hjpeg->Instance->QMEM0));
			}
			else if(hjpeg->Conf.ColorSpace == JPEG_YCBCR_COLORSPACE)
			{
				...
				error =
					JPEG_Set_Quantization_Mem(hjpeg, hjpeg->QuantTable0, (hjpeg->Instance->QMEM0));
				/*By default quantization table 0 for component 0 and quantization table 1 for both
				 * components 1 and 2*/
				error |=
					JPEG_Set_Quantization_Mem(hjpeg, hjpeg->QuantTable1, (hjpeg->Instance->QMEM1));
				...
				if((hjpeg->Context & JPEG_CONTEXT_CUSTOM_TABLES) != 0UL)
				{
					...
					error |= JPEG_Set_Quantization_Mem(hjpeg, hjpeg->QuantTable2,
			...
			else /* ColorSpace == JPEG_CMYK_COLORSPACE */
			{
				...
				if((hjpeg->Context & JPEG_CONTEXT_CUSTOM_TABLES) != 0UL)
				{
					...
					error |= JPEG_Set_Quantization_Mem(hjpeg, hjpeg->QuantTable1,
													   (hjpeg->Instance->QMEM1));
					error |= JPEG_Set_Quantization_Mem(hjpeg, hjpeg->QuantTable2,
													   (hjpeg->Instance->QMEM2));
					error |= JPEG_Set_Quantization_Mem(hjpeg, hjpeg->QuantTable3,
			...
			numberMCU =
				(hMCU * vMCU) - 1UL; /* Bit Field JPEG_CONFR2_NMCU shall be set to NB_MCU - 1*/
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_JPEG_Encode(JPEG_HandleTypeDef* hjpeg, uint8_t* pDataInMCU,
								  ...
								  uint32_t Timeout)
{
	...
	if(hjpeg->State == HAL_JPEG_STATE_READY)
	{
		if((hjpeg->Context & JPEG_CONTEXT_CONF_ENCODING) == JPEG_CONTEXT_CONF_ENCODING)
		{
			...
			hjpeg->InDataLength =
				InDataLength -
				(InDataLength % 4UL); /* In Data length must be multiple of 4 Bytes (1 word)*/
			hjpeg->OutDataLength =
				OutDataLength -
				(OutDataLength % 4UL); /* Out Data length must be multiple of 4 Bytes (1 word)*/
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_JPEG_Decode(JPEG_HandleTypeDef* hjpeg, uint8_t* pDataIn,
								  uint32_t InDataLength, uint8_t* pDataOutMCU,
...
HAL_StatusTypeDef HAL_JPEG_Encode_IT(JPEG_HandleTypeDef* hjpeg, uint8_t* pDataInMCU,
									 uint32_t InDataLength, uint8_t* pDataOut,
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_JPEG_Decode_IT(JPEG_HandleTypeDef* hjpeg, uint8_t* pDataIn,
									 uint32_t InDataLength, uint8_t* pDataOutMCU,
									 uint32_t OutDataLength)
{
	/* Check the parameters */
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_JPEG_Decode_DMA(JPEG_HandleTypeDef* hjpeg, uint8_t* pDataIn,
									  uint32_t InDataLength, uint8_t* pDataOutMCU,
									  uint32_t OutDataLength)
{
	/* Check the parameters */
 ...
 * @retval HAL status
 */
void HAL_JPEG_ConfigInputBuffer(JPEG_HandleTypeDef* hjpeg, uint8_t* pNewInputBuffer,
...
HAL_StatusTypeDef HAL_JPEG_Abort(JPEG_HandleTypeDef* hjpeg)
{
	...
	hjpeg->Context &=
		~(JPEG_CONTEXT_OPERATION_MASK | JPEG_CONTEXT_METHOD_MASK | JPEG_CONTEXT_ENDING_DMA);
 ...
 * @retval None
 */
__weak void HAL_JPEG_DataReadyCallback(JPEG_HandleTypeDef* hjpeg, uint8_t* pDataOut,
									   uint32_t OutDataLength)
{
	/* Prevent unused argument(s) compilation warning */
	UNUSED(hjpeg);
...
static void JPEG_Set_Huff_DHTMem(JPEG_HandleTypeDef* hjpeg)
{
	JPEG_ACHuffTableTypeDef* HuffTableAC0 =
		(JPEG_ACHuffTableTypeDef*)(uint32_t)&JPEG_ACLUM_HuffTable;
	JPEG_ACHuffTableTypeDef* HuffTableAC1 =
		(JPEG_ACHuffTableTypeDef*)(uint32_t)&JPEG_ACCHROM_HuffTable;
	JPEG_DCHuffTableTypeDef* HuffTableDC0 =
		(JPEG_DCHuffTableTypeDef*)(uint32_t)&JPEG_DCLUM_HuffTable;
	JPEG_DCHuffTableTypeDef* HuffTableDC1 =
		(JPEG_DCHuffTableTypeDef*)(uint32_t)&JPEG_DCCHROM_HuffTable;
	...
	/* AC0 VALS is a 162 Bytes table i.e 41x32bits words from DHTMEM base address + 11 to DHTMEM +
	 * 51 */
	...
	*address = value;
	...
	/* DC1 BITS is a 16 Bytes table i.e 4x32bits words from DHTMEM + 51 base address to DHTMEM +
	 * 55*/
	...
	*address = value;
	...
	/* AC1 BITS is a 16 Bytes table i.e 4x32bits words from DHTMEM base address + 58 to DHTMEM +
	 * 62*/
	...
	*address = value;
	...
	/* AC1 VALS is a 162 Bytes table i.e 41x32bits words from DHTMEM base address + 62 to DHTMEM +
	 * 102 */
										  ...
										  __IO uint32_t* QTableAddress)
{
	...
	while(i < (JPEG_QUANT_TABLE_SIZE - 3UL))
	{
		...
		for(j = 0; j < 4UL; j++)
		{
			/* Note that the quantization coefficients must be specified in the table in zigzag
			 * order */
			quantVal =
				((((uint32_t)QTable[JPEG_ZIGZAG_ORDER[i + j]]) * ScaleFactor) + 50UL) / 100UL;
...
static void JPEG_SetColorYCBCR(JPEG_HandleTypeDef* hjpeg)
{
	...
	/*Set Horizental and Vertical  sampling factor , number of blocks , Quantization table and
	 * Huffman AC/DC tables for component 0*/
	hjpeg->Instance->CONFR4 |= (ySamplingH | ySamplingV | (yblockNb & JPEG_CONFR4_NB));

	/*Reset CONFR5 register*/
	hjpeg->Instance->CONFR5 = 0;
	/*Set Horizental and Vertical  sampling factor , number of blocks , Quantization table and
	 * Huffman AC/DC tables for component 1*/
	hjpeg->Instance->CONFR5 |= (JPEG_CONFR5_HSF_0 | JPEG_CONFR5_VSF_0 | JPEG_CONFR5_QT_0 |
	...
	/* In YCBCR , by default, both chrominance components (component 1 and component 2) use the same
	 * Quantization table (table 1) */
	/* In YCBCR , both chrominance components (component 1 and component 2) use the same Huffman
	 * tables (table 1) */
	hjpeg->Instance->CONFR6 |= (JPEG_CONFR6_HSF_0 | JPEG_CONFR6_VSF_0 | JPEG_CONFR6_QT_0 |
...
static void JPEG_Init_Process(JPEG_HandleTypeDef* hjpeg)
{
	...
	if((hjpeg->Context & JPEG_CONTEXT_METHOD_MASK) == JPEG_CONTEXT_IT)
	{
		...
		__HAL_JPEG_ENABLE_IT(hjpeg, JPEG_IT_IFT | JPEG_IT_IFNF | JPEG_IT_OFT | JPEG_IT_OFNE |
										JPEG_IT_EOC | JPEG_IT_HPD);
	}
	else if((hjpeg->Context & JPEG_CONTEXT_METHOD_MASK) == JPEG_CONTEXT_DMA)
	{
...
static uint32_t JPEG_Process(JPEG_HandleTypeDef* hjpeg)
{
	...
	if(__HAL_JPEG_GET_FLAG(hjpeg, JPEG_FLAG_EOCF | JPEG_FLAG_OFTF | JPEG_FLAG_OFNEF) ==
	   JPEG_FLAG_EOCF)
	{
		...
		tmpContext = hjpeg->Context;
		/*Clear all context fields execpt JPEG_CONTEXT_CONF_ENCODING and
	...
	if(hjpeg->OutDataLength >= (hjpeg->JpegOutCount + (nbOutputWords * 4UL)))
	{
		for(index = 0; index < nbOutputWords; index++)
		{
			...
			hjpeg->pJpegOutBuffPtr[hjpeg->JpegOutCount + 1UL] =
				(uint8_t)((dataword & 0x0000FF00UL) >> 8);
			hjpeg->pJpegOutBuffPtr[hjpeg->JpegOutCount + 2UL] =
				(uint8_t)((dataword & 0x00FF0000UL) >> 16);
			hjpeg->pJpegOutBuffPtr[hjpeg->JpegOutCount + 3UL] =
				(uint8_t)((dataword & 0xFF000000UL) >> 24);
...
static HAL_StatusTypeDef JPEG_DMA_StartProcess(JPEG_HandleTypeDef* hjpeg)
{
	...
	/* Start DMA FIFO In transfer */
	if(HAL_DMA_Start_IT(hjpeg->hdmain, (uint32_t)hjpeg->pJpegInBuffPtr,
						(uint32_t)&hjpeg->Instance->DIR, hjpeg->InDataLength >> 2) != HAL_OK)
	{
		hjpeg->ErrorCode |= HAL_JPEG_ERROR_DMA;
		return HAL_ERROR;
	}
	...
	/* Start DMA FIFO Out transfer */
	if(HAL_DMA_Start_IT(hjpeg->hdmaout, (uint32_t)&hjpeg->Instance->DOR,
						(uint32_t)hjpeg->pJpegOutBuffPtr, hjpeg->OutDataLength >> 2) != HAL_OK)
	{
		hjpeg->ErrorCode |= HAL_JPEG_ERROR_DMA;
		return HAL_ERROR;
	}
...
static void JPEG_DMA_EndProcess(JPEG_HandleTypeDef* hjpeg)
{
	uint32_t tmpContext;
	hjpeg->JpegOutCount =
		hjpeg->OutDataLength - ((hjpeg->hdmaout->Instance->NDTR & DMA_SxNDT) << 2);
...
static void JPEG_DMA_PollResidualData(JPEG_HandleTypeDef* hjpeg)
{
	...
	if((__HAL_JPEG_GET_FLAG(hjpeg, JPEG_FLAG_OFNEF) == 0UL) ||
	   ((tmpContext & JPEG_CONTEXT_PAUSE_OUTPUT) == 0UL))
	{
		/*Stop Encoding/Decoding*/
		hjpeg->Instance->CONFR0 &= ~JPEG_CONFR0_START;
...
static void JPEG_DMAInCpltCallback(DMA_HandleTypeDef* hdma)
{
	...
	/* Disable The JPEG IT so the DMA Input Callback can not be interrupted by the JPEG EOC IT or
	 * JPEG HPD IT */
	__HAL_JPEG_DISABLE_IT(hjpeg, JPEG_INTERRUPT_MASK);
...
static void JPEG_DMAOutCpltCallback(DMA_HandleTypeDef* hdma)
{
	...
	/* Disable The JPEG IT so the DMA Output Callback can not be interrupted by the JPEG EOC IT or
	 * JPEG HPD IT */
	__HAL_JPEG_DISABLE_IT(hjpeg, JPEG_INTERRUPT_MASK);
	...
	while(i < (JPEG_QUANT_TABLE_SIZE - 3UL))
	{
		...
		for(j = 0; j < 4UL; j++)
		{
			...
			else
			{
				 ...
				 * order */
				scale =
					(quantVal * 100UL) / ((uint32_t)hjpeg->QuantTable0[JPEG_ZIGZAG_ORDER[i + j]]);
