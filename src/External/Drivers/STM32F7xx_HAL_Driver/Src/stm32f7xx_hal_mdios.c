/**
 ...
 ===============================================================================
	...
	(#) Enable interrupts on events using HAL_MDIOS_EnableEvents(), when called
	   ...
	   -@@- A callback is executed for each genereted interrupt, so the driver provides the
 following HAL_MDIOS_WriteCpltCallback(), HAL_MDIOS_ReadCpltCallback() and HAL_MDIOS_ErrorCallback()
