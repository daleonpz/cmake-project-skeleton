/**
 ...
 one. User must resort to HAL_HASH_xxx_Accumulate_End() to enter the last one and retrieve as well
 the computed digest.

		(##) In interrupt mode, API HAL_HASH_xxx_Accumulate_IT() must be called for each input
 buffer, except for the last one. User must resort to HAL_HASH_xxx_Accumulate_End_IT() to enter the
 ...
 through the same API HAL_HASH_xxx_Start_DMA(). The digest can then be retrieved with a call to API
 HAL_HASH_xxx_Finish().
			 ...
			 in the Peripheral with the API HAL_HMACEx_xxx_Step1_2_DMA(). This carries out HMAC step
 1 and starts step 2. The following buffers are next entered with the API
 HAL_HMACEx_xxx_Step2_DMA(). At this point, the HMAC processing is still carrying out step 2. Then,
 step 2 for the last input buffer and step 3 are carried out by a single call to
 HAL_HMACEx_xxx_Step2_3_DMA().
 ...
 suspension time, processing can be restarted with the same API call (same API, same handle, same
 parameters) as done before the suspension. Relevant parameters to restart at the proper location
 are internally saved in the HASH handle.
	 ...
	 *** Remarks on message length ***
	 ...
	 [..]
	  (#) HAL in interruption mode (interruptions driven)
		...
		(##)Due to HASH peripheral hardware design, the peripheral interruption is triggered every
 64 bytes. This is why, for driver implementation simplicity’s sake, user is requested to enter a
 message the length of which is a multiple of 4 bytes.

		(##) When the message length (in bytes) is not a multiple of words, a specific field exists
 in HASH_STR to specify which bits to discard at the end of the complete message to process only the
 message bits and not extra bits.

		(##) If user needs to perform a hash computation of a large input buffer that is spread
 around various places in memory and where each piece of this input buffer is not necessarily a
 multiple of 4 bytes in size, it becomes necessary to use a temporary buffer to format the data
 ...
 word-basis. The same field described above in HASH_STR is used to specify which bits to discard at
 the end of the DMA transfer to process only the message bits and not extra bits. Due to hardware
 implementation, this is possible only at the end of the complete message. When several DMA
 transfers are needed to enter the message, this is not applicable at the end of the intermediary
 transfers.

		(##) Similarly to the interruption-driven mode, it is suggested to the user to format the
 consecutive chunks of data by software while the DMA transfer and processing is on-going for the
 first parts of the message. Due to the 32-bit alignment required for the DMA transfer, it is
 underlined that the software formatting operation is more complex than in the IT mode.
...
/**
 * @brief  End computation of a single HASH signature after several calls to HAL_HASH_MD5_Accmlt()
 * API.
 * @note   Digest is available in pOutBuffer.
 * @param  hhash HASH handle.
 * @param  pInBuffer pointer to the input buffer (buffer to be hashed).
...
/**
 * @brief  End computation of a single HASH signature after several calls to HAL_HASH_SHA1_Accmlt()
 * API.
 * @note   Digest is available in pOutBuffer.
 * @param  hhash HASH handle.
 * @param  pInBuffer pointer to the input buffer (buffer to be hashed).
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_MD5_Accmlt_End_IT(HASH_HandleTypeDef* hhash, uint8_t* pInBuffer,
											 uint32_t Size, uint8_t* pOutBuffer)
{
	return HASH_Start_IT(hhash, pInBuffer, Size, pOutBuffer, HASH_ALGOSELECTION_MD5);
}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_HASH_SHA1_Accmlt_End_IT(HASH_HandleTypeDef* hhash, uint8_t* pInBuffer,
											  uint32_t Size, uint8_t* pOutBuffer)
{
	return HASH_Start_IT(hhash, pInBuffer, Size, pOutBuffer, HASH_ALGOSELECTION_SHA1);
}
	...
	if(hhash->State != HAL_HASH_STATE_SUSPENDED)
	{
		...
		else
		{
			...
			else if(hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_1)
			{
				...
				/* Check whether or not digest calculation must be disabled (in case of multi-buffer
				 * HMAC processing) */
				if(hhash->DigestCalculationDisable != RESET)
				{
					/* Digest calculation is disabled: Step 2 must start with MDMAT bit set,
					no digest calculation will be triggered at the end of the input buffer feeding
					to the Peripheral */
			...
			else /*case (hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_2)*/
			{
				if(hhash->DigestCalculationDisable != RESET)
				{
					/* No automatic move to Step 3 as a new message buffer will be fed to the
					Peripheral (case of multi-buffer HMAC processing): DCAL must not be set. Phase
					remains in Step 2, MDMAT remains set at this point. Change the HASH state to
					ready and call Input data transfer complete call back. */
					hhash->State = HAL_HASH_STATE_READY;
		#if(USE_HAL_HASH_REGISTER_CALLBACKS == 1)
					hhash->InCpltCallback(hhash);
		#else
				...
				{
					/* Digest calculation is not disabled (case of single buffer input or last
					buffer of multi-buffer HMAC processing) */
					inputaddr = (uint32_t)hhash->Init.pKey; /* DMA transfer start address */
					buffersize = hhash->Init.KeySize; /* DMA transfer size (in bytes) */
					hhash->Phase =
						HAL_HASH_PHASE_HMAC_STEP_3; /* Move phase from Step 2 to Step 3 */
					/* In case of suspension request, save the new starting parameters */
					hhash->HashInCount =
	...
	for(buffercounter = 0U; buffercounter < Size; buffercounter += 4U)
	{
		...
		if((hhash->SuspendRequest == HAL_HASH_SUSPEND) && ((buffercounter + 4U) < Size))
		{
			...
			if(__HAL_HASH_GET_FLAG(HASH_FLAG_DINIS))
			{
				...
				/* Depending whether the key or the input data were fed to the Peripheral, the
				feeding point reached at suspension time is not saved in the same handle fields */
		...
		if(__HAL_HASH_GET_FLAG(HASH_FLAG_DINIS))
		{
			...
			if(HASH_Write_Block_Data(hhash) == HASH_DIGEST_CALCULATION_STARTED)
			{
				...
				if(hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_1)
				{
					...
					hhash->Phase =
						HAL_HASH_PHASE_HMAC_STEP_2; /* Move phase from Step 1 to Step 2 */
					__HAL_HASH_SET_NBVALIDBITS(
						hhash->HashBuffSize); /* Set NBLW for the input message */
					hhash->HashInCount =
						hhash->HashBuffSize; /* Set the input data size (in bytes) */
...
static uint32_t HASH_Write_Block_Data(HASH_HandleTypeDef* hhash)
{
	...
	else
	{
		...
		{
			/* Field accumulation is set, API only feeds data to the Peripheral and under
			   interruption. The digest computation will be started when the last buffer data are
			   entered. */
...
static HAL_StatusTypeDef HMAC_Processing(HASH_HandleTypeDef* hhash, uint32_t Timeout)
{
	...
	if((hhash->Phase != HAL_HASH_PHASE_HMAC_STEP_1) &&
	   (hhash->Phase != HAL_HASH_PHASE_HMAC_STEP_2) && (hhash->Phase != HAL_HASH_PHASE_HMAC_STEP_3))
	{
		/* Change the HASH state */
		hhash->State = HAL_HASH_STATE_READY;
	...
	if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
	{
		...
		if(hhash->State == HAL_HASH_STATE_SUSPENDED)
		{
			...
			/* Since this is resumption, pInBuffer_tmp and Size_tmp are not set
			   ...
			   when the processing was suspended */
			pInBuffer_tmp =
				hhash->pHashInBuffPtr; /* pInBuffer_tmp is set to the input data address */
		...
		else
		{
			...
			if(hhash->Phase == HAL_HASH_PHASE_READY)
			{
				/* Select the HASH algorithm, clear HMAC mode and long key selection bit, reset the
				 * HASH processor core */
				MODIFY_REG(HASH->CR, HASH_CR_LKEY | HASH_CR_ALGO | HASH_CR_MODE | HASH_CR_INIT,
						   Algorithm | HASH_CR_INIT);
			}
								...
								uint8_t* pOutBuffer, uint32_t Algorithm)
{
	...
	if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
	{
		...
		if(hhash->Phase == HAL_HASH_PHASE_READY)
		{
			...
			hhash->HashInCount = SizeVar; /* Counter used to keep track of number of data
											 to be fed to the Peripheral */
			hhash->pHashInBuffPtr = pInBuffer; /* Points at data which will be fed to the Peripheral
							 ...
							 uint8_t* pOutBuffer, uint32_t Timeout, uint32_t Algorithm)
{
	...
	if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
	{
		...
		if(hhash->Phase == HAL_HASH_PHASE_READY)
		{
			/* Check if key size is larger than 64 bytes, accordingly set LKEY and the other setting
			 * bits */
			if(hhash->Init.KeySize > 64U)
			{
				MODIFY_REG(HASH->CR, HASH_CR_LKEY | HASH_CR_ALGO | HASH_CR_MODE | HASH_CR_INIT,
						   Algorithm | HASH_ALGOMODE_HMAC | HASH_HMAC_KEYTYPE_LONGKEY |
							   HASH_CR_INIT);
			}
			else
			{
			...
			hhash->pHashKeyBuffPtr = hhash->Init.pKey; /* Key address, HMAC_Processing input
														  parameter for Step 1 and Step 3 */
			hhash->HashKeyCount = hhash->Init.KeySize; /* Key size, HMAC_Processing input parameter
	...
	if((State_tmp == HAL_HASH_STATE_READY) || (State_tmp == HAL_HASH_STATE_SUSPENDED))
	{
		...
		if(hhash->State == HAL_HASH_STATE_READY)
		{
			...
			if(hhash->Phase == HAL_HASH_PHASE_READY)
			{
				/* Change the HASH state */
				hhash->State = HAL_HASH_STATE_BUSY;
				/* Check if key size is larger than 64 bytes, accordingly set LKEY and the other
				   setting bits. At the same time, ensure MDMAT bit is cleared. */
				if(hhash->Init.KeySize > 64U)
				{
					MODIFY_REG(
						HASH->CR,
				...
				/* Store input aparameters in handle fields to manage steps transition
				   or possible HMAC suspension/resumption */
				hhash->HashInCount =
					hhash->Init.KeySize; /* Initial size for first DMA transfer (key size)      */
				hhash->pHashKeyBuffPtr = hhash->Init.pKey; /* Key address */
				hhash->pHashInBuffPtr =
				...
				inputaddr = (uint32_t)(hhash->Init.pKey); /* Address passed to DMA (start by
															 entering Key message) */
			...
			else if(hhash->Phase == HAL_HASH_PHASE_HMAC_STEP_2)
			{
				...
				if(hhash->DigestCalculationDisable == RESET)
				{
					/* This means this is the last buffer of the multi-buffer sequence: DCAL needs
					 * to be set. */
					__HAL_HASH_RESET_MDMAT();
