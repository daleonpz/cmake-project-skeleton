 ...
 */
	...
	#define USART_CR1_FIELDS                                                                   \
		((uint32_t)(USART_CR1_M | USART_CR1_PCE | USART_CR1_PS | USART_CR1_TE | USART_CR1_RE | \
					USART_CR1_OVER8)) /*!< USART CR1 fields of parameters set by               \
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Transmit(SMARTCARD_HandleTypeDef* hsmartcard, const uint8_t* pData,
										 uint16_t Size, uint32_t Timeout)
{
	...
	if(hsmartcard->gState == HAL_SMARTCARD_STATE_READY)
	{
		...
		while(hsmartcard->TxXferCount > 0U)
		{
			...
			if(SMARTCARD_WaitOnFlagUntilTimeout(hsmartcard, SMARTCARD_FLAG_TXE, RESET, tickstart,
												Timeout) != HAL_OK)
			{
				return HAL_TIMEOUT;
			}
		...
		if(SMARTCARD_WaitOnFlagUntilTimeout(hsmartcard,
...
{
	/* Disable RTOIE, EOBIE, TXEIE, TCIE, RXNE, PE and ERR (Frame error, noise error, overrun error)
	 * interrupts */
...
HAL_StatusTypeDef HAL_SMARTCARD_AbortReceive(SMARTCARD_HandleTypeDef* hsmartcard)
{
	...
	CLEAR_BIT(hsmartcard->Instance->CR1,
			  (USART_CR1_RXNEIE | USART_CR1_PEIE | USART_CR1_RTOIE | USART_CR1_EOBIE));
	CLEAR_BIT(hsmartcard->Instance->CR3, USART_CR3_EIE);
...
void HAL_SMARTCARD_IRQHandler(SMARTCARD_HandleTypeDef* hsmartcard)
{
	...
	if((errorflags != 0U) &&
	   (((cr3its & USART_CR3_EIE) != 0U) || ((cr1its & (USART_CR1_RXNEIE | USART_CR1_PEIE)) != 0U)))
	{
			...
			*/
			   ...
			   ((errorcode & (HAL_SMARTCARD_ERROR_RTO | HAL_SMARTCARD_ERROR_ORE)) != 0U))
			{
				...
				if(HAL_IS_BIT_SET(hsmartcard->Instance->CR3, USART_CR3_DMAR))
				{
					...
					{
						/* Set the SMARTCARD DMA Abort callback :
						   will lead to call HAL_SMARTCARD_ErrorCallback() at end of DMA abort
						   procedure */
	...
	#endif /* USE_HAL_SMARTCARD_REGISTER_CALLBACK */
		/* Clear EOBF interrupt after HAL_SMARTCARD_RxCpltCallback() call for the End of Block
		   information to be available during HAL_SMARTCARD_RxCpltCallback() processing */
	...
	{
		if(__HAL_SMARTCARD_GET_IT_SOURCE(hsmartcard,
										 hsmartcard->AdvancedInit.TxCompletionIndication) != RESET)
		{
...
static HAL_StatusTypeDef SMARTCARD_SetConfig(SMARTCARD_HandleTypeDef* hsmartcard)
{
	...
	tmpreg =
		(hsmartcard->Init.Prescaler | ((uint32_t)hsmartcard->Init.GuardTime << USART_GTPR_GT_Pos));
	...
	switch(clocksource)
	{
		case SMARTCARD_CLOCKSOURCE_PCLK1:
			...
			tmpreg =
				(uint32_t)((pclk + (hsmartcard->Init.BaudRate / 2U)) / hsmartcard->Init.BaudRate);
			break;
		...
		case SMARTCARD_CLOCKSOURCE_HSI:
			tmpreg = (uint32_t)((HSI_VALUE + (hsmartcard->Init.BaudRate / 2U)) /
		...
		case SMARTCARD_CLOCKSOURCE_LSE:
			tmpreg = (uint32_t)((LSE_VALUE + (hsmartcard->Init.BaudRate / 2U)) /
								hsmartcard->Init.BaudRate);
			break;
		default:
			ret = HAL_ERROR;
...
static void SMARTCARD_AdvFeatureConfig(SMARTCARD_HandleTypeDef* hsmartcard)
{
	...
	{
		assert_param(
			IS_SMARTCARD_ADVFEATURE_DMAONRXERROR(hsmartcard->AdvancedInit.DMADisableonRxError));
...
static void SMARTCARD_EndTransmit_IT(SMARTCARD_HandleTypeDef* hsmartcard)
{
	...
	if((hsmartcard->Init.Mode == SMARTCARD_MODE_TX_RX) ||
	   (hsmartcard->Init.NACKEnable == SMARTCARD_NACK_ENABLE))
	{
		/* Perform a TX FIFO Flush at end of Tx phase, as all sent bytes are appearing in Rx Data
		 * register */
		__HAL_SMARTCARD_FLUSH_DRREGISTER(hsmartcard);
	}
	SET_BIT(hsmartcard->Instance->CR1, USART_CR1_UE);
