...
HAL_StatusTypeDef HAL_SAI_Init(SAI_HandleTypeDef* hsai)
{
	...
	if((hsai->Init.AudioMode == SAI_MODEMASTER_TX) || (hsai->Init.AudioMode == SAI_MODESLAVE_TX))
	{
		/* Transmit */
		ckstr_bits =
			(hsai->Init.ClockStrobing == SAI_CLOCKSTROBING_RISINGEDGE) ? 0 : SAI_xCR1_CKSTR;
	}
	else
	{
		/* Receive */
		ckstr_bits =
			(hsai->Init.ClockStrobing == SAI_CLOCKSTROBING_RISINGEDGE) ? SAI_xCR1_CKSTR : 0;
	}
	...
	hsai->Instance->FRCR |= ((hsai->FrameInit.FrameLength - 1) | hsai->FrameInit.FSOffset |
