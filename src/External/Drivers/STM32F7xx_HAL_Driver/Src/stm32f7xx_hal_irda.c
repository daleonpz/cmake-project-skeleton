 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_IRDA_Transmit(IRDA_HandleTypeDef* hirda, const uint8_t* pData, uint16_t Size,
									uint32_t Timeout)
{
										...
										uint16_t Size)
{
	...
	if(hirda->gState == HAL_IRDA_STATE_READY)
	{
		...
		if(HAL_DMA_Start_IT(hirda->hdmatx, (uint32_t)hirda->pTxBuffPtr,
							(uint32_t)&hirda->Instance->TDR, Size) == HAL_OK)
		{
...
HAL_StatusTypeDef HAL_IRDA_Receive_DMA(IRDA_HandleTypeDef* hirda, uint8_t* pData, uint16_t Size)
{
	...
	if(hirda->RxState == HAL_IRDA_STATE_READY)
	{
		...
		if(HAL_DMA_Start_IT(hirda->hdmarx, (uint32_t)&hirda->Instance->RDR,
							(uint32_t)hirda->pRxBuffPtr, Size) == HAL_OK)
		{
			/* Process Unlocked */
			__HAL_UNLOCK(hirda);
...
HAL_StatusTypeDef HAL_IRDA_Abort_IT(IRDA_HandleTypeDef* hirda)
{
	...
	/* If DMA Tx and/or DMA Rx Handles are associated to IRDA Handle, DMA Abort complete callbacks
...
HAL_StatusTypeDef HAL_IRDA_AbortReceive_IT(IRDA_HandleTypeDef* hirda)
{
	...
	if(HAL_IS_BIT_SET(hirda->Instance->CR3, USART_CR3_DMAR))
	{
		...
		else
		{
			...
			/* Clear the Error flags in the ICR register */
			__HAL_IRDA_CLEAR_FLAG(hirda, IRDA_CLEAR_OREF | IRDA_CLEAR_NEF | IRDA_CLEAR_PEF |
											 IRDA_CLEAR_FEF);
