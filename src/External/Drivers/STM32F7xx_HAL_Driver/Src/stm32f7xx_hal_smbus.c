											   ...
											   uint8_t* pData, uint16_t Size, uint32_t XferOptions)
{
	...
	if(hsmbus->State == HAL_SMBUS_STATE_READY)
	{
		...
		else
		{
			...
			/* If PEC mode is enable, size to transmit manage by SW part should be Size-1 byte,
											 ...
											 uint16_t Size, uint32_t XferOptions)
{
	...
	if((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN)
	{
		...
		/* If XferSize equal "1", or XferSize equal "2" with PEC requested (mean 1 data byte + 1 PEC
		 * byte */
	...
	else if(SMBUS_CHECK_FLAG(StatusFlags, SMBUS_FLAG_TCR) != RESET)
	{
		if((hsmbus->XferCount != 0U) && (hsmbus->XferSize == 0U))
		{
			...
			else
			{
				...
				SMBUS_TransferConfig(hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
									 hsmbus->XferOptions, SMBUS_NO_STARTSTOP);
				/* If PEC mode is enable, size to transmit should be Size-1 byte, corresponding to
				 * PEC byte */
				/* PEC byte is automatically sent by HW block, no need to manage it in Transmit
				 * process */
				if(SMBUS_GET_PEC_MODE(hsmbus) != 0UL)
				{
					hsmbus->XferSize--;
		...
		else if((hsmbus->State & HAL_SMBUS_STATE_SLAVE_BUSY_TX) == HAL_SMBUS_STATE_SLAVE_BUSY_TX)
		{
			if((hsmbus->XferCount != 0U) && (hsmbus->XferSize == 0U))
			{
				...
				else
				{
					...
					SMBUS_TransferConfig(hsmbus, 0, (uint8_t)hsmbus->XferSize, hsmbus->XferOptions,
										 SMBUS_NO_STARTSTOP);
					/* If PEC mode is enable, size to transmit should be Size-1 byte, corresponding
					 * to PEC byte */
					/* PEC byte is automatically sent by HW block, no need to manage it in Transmit
					 * process */
					if(SMBUS_GET_PEC_MODE(hsmbus) != 0UL)
