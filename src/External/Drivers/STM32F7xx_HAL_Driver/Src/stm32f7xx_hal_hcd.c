	...
	else if(__HAL_HCD_GET_CH_FLAG(hhcd, ch_num, USB_OTG_HCINT_XFRC))
	{
		...
		if(hhcd->Init.dma_enable != 0U)
		{
			hhcd->hc[ch_num].xfer_count =
				hhcd->hc[ch_num].XferSize - (USBx_HC(ch_num)->HCTSIZ & USB_OTG_HCTSIZ_XFRSIZ);
		}
	...
	else if(__HAL_HCD_GET_CH_FLAG(hhcd, ch_num, USB_OTG_HCINT_CHH))
	{
		...
		if(hhcd->hc[ch_num].state == HC_XFRC)
		{
			   ...
			   (hhcd->hc[ch_num].ep_type == EP_TYPE_INTR))
			{
				...
				{
					num_packets = (hhcd->hc[ch_num].xfer_len + hhcd->hc[ch_num].max_packet - 1U) /
								  hhcd->hc[ch_num].max_packet;
	...
	switch(pktsts)
	{
		case GRXSTS_PKTSTS_IN:
			...
			if((pktcnt > 0U) && (hhcd->hc[ch_num].xfer_buff != (void*)0))
			{
				...
				{
					(void)USB_ReadPacket(hhcd->Instance, hhcd->hc[ch_num].xfer_buff,
										 (uint16_t)pktcnt);
