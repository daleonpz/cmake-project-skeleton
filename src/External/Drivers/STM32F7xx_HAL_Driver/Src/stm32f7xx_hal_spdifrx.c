/**
  ...
  *** Callback registration ***
  ...
  If not, MspInit or MspDeInit callbacks pointers are not null, the HAL_SPDIFRX_Init() /
 HAL_SPDIFRX_DeInit() keep and use the user MspInit/MspDeInit functions (registered beforehand)
...
HAL_StatusTypeDef HAL_SPDIFRX_Init(SPDIFRX_HandleTypeDef* hspdif)
{
	...
	if(hspdif->State == HAL_SPDIFRX_STATE_RESET)
	{
		...
		hspdif->RxHalfCpltCallback =
			HAL_SPDIFRX_RxHalfCpltCallback; /* Legacy weak RxHalfCpltCallback */
		hspdif->RxCpltCallback = HAL_SPDIFRX_RxCpltCallback; /* Legacy weak RxCpltCallback     */
		hspdif->CxHalfCpltCallback =
			HAL_SPDIFRX_CxHalfCpltCallback; /* Legacy weak CxHalfCpltCallback */
	...
	tmpreg &= ~(SPDIFRX_CR_RXSTEO | SPDIFRX_CR_DRFMT | SPDIFRX_CR_PMSK | SPDIFRX_CR_VMSK |
				SPDIFRX_CR_CUMSK | SPDIFRX_CR_PTMSK | SPDIFRX_CR_CHSEL | SPDIFRX_CR_NBTR |
				SPDIFRX_CR_WFA | SPDIFRX_CR_INSEL);

	/* Sets the new configuration of the SPDIFRX peripheral */
	tmpreg |=
		(hspdif->Init.StereoMode | hspdif->Init.InputSelection | hspdif->Init.Retries |
...
/**
 * @brief Set the SPDIFRX  data format according to the specified parameters in the
 * SPDIFRX_InitTypeDef.
 * @param hspdif SPDIFRX handle
											...
											SPDIFRX_SetDataFormatTypeDef sDataFormat)
{
	...
	tmpreg &= ~(SPDIFRX_CR_RXSTEO | SPDIFRX_CR_DRFMT | SPDIFRX_CR_PMSK | SPDIFRX_CR_VMSK |
				SPDIFRX_CR_CUMSK | SPDIFRX_CR_PTMSK);

	/* Configure the new data format */
	tmpreg |=
		(sDataFormat.StereoMode | sDataFormat.DataFormat | sDataFormat.PreambleTypeMask |
											  ...
											  uint16_t Size, uint32_t Timeout)
{
	...
	if(hspdif->State == HAL_SPDIFRX_STATE_READY)
	{
		...
		while(sizeCounter > 0U)
		{
			...
			if(SPDIFRX_WaitOnFlagUntilTimeout(hspdif, SPDIFRX_FLAG_RXNE, RESET, Timeout,
											  tickstart) != HAL_OK)
			{
				return HAL_TIMEOUT;
			}
												 ...
												 uint16_t Size, uint32_t Timeout)
{
	...
	if(hspdif->State == HAL_SPDIFRX_STATE_READY)
	{
		...
		while(sizeCounter > 0U)
		{
			...
			if(SPDIFRX_WaitOnFlagUntilTimeout(hspdif, SPDIFRX_FLAG_CSRNE, RESET, Timeout,
											  tickstart) != HAL_OK)
			{
				return HAL_TIMEOUT;
			}
