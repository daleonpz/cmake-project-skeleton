												   ...
												   uint32_t Channel)
{
	   ...
	   (hdfsdm_filter->State != HAL_DFSDM_FILTER_STATE_ERROR))
	{
		...
		hdfsdm_filter->InjConvRemaining =
			(hdfsdm_filter->InjectedScanMode == ENABLE) ? hdfsdm_filter->InjectedChannelsNbr : 1;
	}
	else
	{
		status = HAL_ERROR;
												   ...
												   int32_t* pData, uint32_t Length)
{
	...
	/* Check parameters compatibility */
			...
			(hdfsdm_filter->RegularContMode == DFSDM_CONTINUOUS_CONV_OFF) &&
			(hdfsdm_filter->hdmaReg->Init.Mode == DMA_NORMAL) && (Length != 1))
	{
		status = HAL_ERROR;
	}
	else if((hdfsdm_filter->RegularTrigger == DFSDM_FILTER_SW_TRIGGER) &&
													  ...
													  int16_t* pData, uint32_t Length)
{
			...
			(hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_INJ))
	{
		...
		if(HAL_DMA_Start_IT(hdfsdm_filter->hdmaReg,
							(uint32_t)(&hdfsdm_filter->Instance->FLTRDATAR) + 2, (uint32_t)pData,
							Length) != HAL_OK)
		{
			/* Set DFSDM filter in error state */
			hdfsdm_filter->State = HAL_DFSDM_FILTER_STATE_ERROR;
													   ...
													   int16_t* pData, uint32_t Length)
{
			...
			(hdfsdm_filter->State == HAL_DFSDM_FILTER_STATE_REG))
	{
		...
		if(HAL_DMA_Start_IT(hdfsdm_filter->hdmaInj,
							(uint32_t)(&hdfsdm_filter->Instance->FLTJDATAR) + 2, (uint32_t)pData,
							Length) != HAL_OK)
		{
			/* Set DFSDM filter in error state */
			hdfsdm_filter->State = HAL_DFSDM_FILTER_STATE_ERROR;
...
void HAL_DFSDM_IRQHandler(DFSDM_Filter_HandleTypeDef* hdfsdm_filter)
{
			...
			((hdfsdm_filter->Instance->FLTCR2 & DFSDM_FLTCR2_AWDIE) != 0))
	{
		...
		threshold =
			((reg & DFSDM_FLTAWSR_AWLTF) != 0) ? DFSDM_AWD_LOW_THRESHOLD : DFSDM_AWD_HIGH_THRESHOLD;
