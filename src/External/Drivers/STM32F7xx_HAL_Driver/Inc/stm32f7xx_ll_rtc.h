...
#include "stm32f7xx.h"
	...
	#define LL_RTC_TAMPER_FILTER_2SAMPLE                                                        \
		RTC_TAMPCR_TAMPFLT_0 /*!< Tamper is activated after 2 consecutive samples at the active \
								level */
	#define LL_RTC_TAMPER_FILTER_4SAMPLE                                                        \
		RTC_TAMPCR_TAMPFLT_1 /*!< Tamper is activated after 4 consecutive samples at the active \
								level */
	...
	#define LL_RTC_TAMPER_SAMPLFREQDIV_16384                                                       \
		RTC_TAMPCR_TAMPFREQ_0 /*!< Each of the tamper inputs are sampled with a frequency = RTCCLK \
								 / 16384 */
	#define LL_RTC_TAMPER_SAMPLFREQDIV_8192                                                        \
		RTC_TAMPCR_TAMPFREQ_1 /*!< Each of the tamper inputs are sampled with a frequency = RTCCLK \
								 / 8192 */
	#define LL_RTC_TAMPER_SAMPLFREQDIV_4096                                                        \
		(RTC_TAMPCR_TAMPFREQ_1 | RTC_TAMPCR_TAMPFREQ_0) /*!< Each of the tamper inputs are sampled \
														   with a frequency =  RTCCLK / 4096 */
	#define LL_RTC_TAMPER_SAMPLFREQDIV_2048                                                        \
		RTC_TAMPCR_TAMPFREQ_2 /*!< Each of the tamper inputs are sampled with a frequency = RTCCLK \
								 / 2048 */
	#define LL_RTC_TAMPER_SAMPLFREQDIV_1024                                                        \
		(RTC_TAMPCR_TAMPFREQ_2 | RTC_TAMPCR_TAMPFREQ_0) /*!< Each of the tamper inputs are sampled \
														   with a frequency =  RTCCLK / 1024 */
	#define LL_RTC_TAMPER_SAMPLFREQDIV_512                                                         \
		(RTC_TAMPCR_TAMPFREQ_2 | RTC_TAMPCR_TAMPFREQ_1) /*!< Each of the tamper inputs are sampled \
														   with a frequency =  RTCCLK / 512 */
	#define LL_RTC_TAMPER_SAMPLFREQDIV_256                                                        \
		RTC_TAMPCR_TAMPFREQ /*!< Each of the tamper inputs are sampled with a frequency =  RTCCLK \
							   / 256 */
	/**
	 * @}
	 */
	...
	#define LL_RTC_TAMPER_ACTIVELEVEL_TAMP1                                                     \
		RTC_TAMPCR_TAMP1TRG /*!< RTC_TAMP1 input falling edge (if TAMPFLT = 00) or staying high \
							   (if TAMPFLT != 00) triggers a tamper detection event */
	#define LL_RTC_TAMPER_ACTIVELEVEL_TAMP2                                                     \
		RTC_TAMPCR_TAMP2TRG /*!< RTC_TAMP2 input falling edge (if TAMPFLT = 00) or staying high \
							   (if TAMPFLT != 00) triggers a tamper detection event */
	#define LL_RTC_TAMPER_ACTIVELEVEL_TAMP3                                                     \
		RTC_TAMPCR_TAMP3TRG /*!< RTC_TAMP3 input falling edge (if TAMPFLT = 00) or staying high \
							   (if TAMPFLT != 00) triggers a tamper detection event */
	/**
	 * @}
	 */
	 ...
	 * @param  RTCx RTC Instance
	 * @retval Value between Min_Data=0x01 and Max_Data=0x12 or between Min_Data=0x00 and
	 * Max_Data=0x23
	 */
	 ...
	 * before.
	 * @note   It can be written in initialization mode only (@ref LL_RTC_EnableInitMode function)
	 * @note helper macro __LL_RTC_CONVERT_BIN2BCD is available to convert Minutes from binary to
	 * BCD format
	 ...
	 * before.
	 * @note   It can be written in initialization mode only (@ref LL_RTC_EnableInitMode function)
	 * @note helper macro __LL_RTC_CONVERT_BIN2BCD is available to convert Seconds from binary to
	 * BCD format
	 ...
	 * @note   This operation effectively subtracts from (delays) or advance the clock of a fraction
	 * of a second.
	 * @note   Bit is write-protected. @ref LL_RTC_DisableWriteProtection function should be called
	 * before.
	 ...
	 * @note helper macro __LL_RTC_CONVERT_BIN2BCD is available to convert Day from binary to BCD
	 * format
	 ...
	 * @note if shadow mode is disabled (BYPSHAD=0), need to check if RSF flag is set
	 *       before reading this bit
	 * @note helper macro __LL_RTC_CONVERT_BCD2BIN is available to convert Day from BCD to Binary
	 * format
	...
	__STATIC_INLINE uint32_t LL_RTC_DATE_Get(RTC_TypeDef* RTCx)
	{
		...
		return (
			uint32_t)((((temp & RTC_DR_WDU) >> RTC_DR_WDU_Pos) << RTC_OFFSET_WEEKDAY) |
	 ...
	 * @note helper macro __LL_RTC_CONVERT_BCD2BIN is available to convert Minutes from BCD to
	 * Binary format
	 ...
	 * @note helper macro __LL_RTC_CONVERT_BCD2BIN is available to convert Seconds from BCD to
	 * Binary format
	...
	/**
	 * @brief  Check if one RTCCLK has been inserted or not every 2exp11 pulses (frequency increased
	 * by 488.5 ppm)
