...
extern "C"
{
	...
	{
		uint32_t
			Type; /*!< SPI or Manchester modes.
					   This parameter can be a value of @ref DFSDM_Channel_SerialInterfaceType */
		uint32_t
			SpiClock; /*!< SPI clock select (external or internal with different sampling point).
	...
	{
		uint32_t
			FilterOrder; /*!< Analog watchdog Sinc filter order.
	...
	{
		uint32_t
			Trigger; /*!< Trigger used to start injected conversion: software, external or
	...
	#define DFSDM_CHANNEL_OUTPUT_CLOCK_SYSTEM \
		((uint32_t)0x00000000U) /*!< Source for output clock is system clock */
	...
	#define DFSDM_CHANNEL_EXTERNAL_INPUTS \
		((uint32_t)0x00000000U) /*!< Data are taken from external inputs */
	...
	#define DFSDM_CHANNEL_SAME_CHANNEL_PINS \
		((uint32_t)0x00000000U) /*!< Input from pins on same channel */
	...
	#define DFSDM_CHANNEL_MANCHESTER_RISING                   \
		DFSDM_CHCFGR1_SITP_1 /*!< Manchester with rising edge \
							  */
	#define DFSDM_CHANNEL_MANCHESTER_FALLING                 \
		DFSDM_CHCFGR1_SITP /*!< Manchester with falling edge \
							*/
	/**
	 * @}
	 */
	...
	#define DFSDM_FILTER_EXT_TRIGGER \
		((uint32_t)0x00000002U) /*!< External trigger (only for injected conversion) */
	/**
	 * @}
	 */
	...
	#define DFSDM_FILTER_EXT_TRIG_TIM1_TRGO \
		((uint32_t)0x00000000U) /*!< For DFSDM filter 0, 1, 2 and 3 */
	...
	#define DFSDM_FILTER_EXT_TRIG_TIM8_TRGO2 \
		(DFSDM_FLTCR1_JEXTSEL_0 | DFSDM_FLTCR1_JEXTSEL_1) /*!< For DFSDM filter 0, 1, 2 and 3 */
	...
	#define DFSDM_FILTER_EXT_TRIG_TIM4_TRGO \
		(DFSDM_FLTCR1_JEXTSEL_0 | DFSDM_FLTCR1_JEXTSEL_2) /*!< For DFSDM filter 0, 1, 2 and 3 */
	...
	#define DFSDM_FILTER_EXT_TRIG_TIM7_TRGO \
		DFSDM_FLTCR1_JEXTSEL_3 /*!< For DFSDM filter 0, 1, 2 and 3 */
	#define DFSDM_FILTER_EXT_TRIG_EXTI11 \
		(DFSDM_FLTCR1_JEXTSEL_3 | DFSDM_FLTCR1_JEXTSEL_4) /*!< For DFSDM filter 0, 1, 2 and 3 */
	...
	#define DFSDM_FILTER_ERROR_REGULAR_OVERRUN \
		((uint32_t)0x00000001U) /*!< Overrun occurs during regular conversion */
	#define DFSDM_FILTER_ERROR_INJECTED_OVERRUN \
		((uint32_t)0x00000002U) /*!< Overrun occurs during injected conversion */
	#define DFSDM_FILTER_ERROR_DMA ((uint32_t)0x00000003U) /*!< DMA error occurs */
	#if(USE_HAL_DFSDM_REGISTER_CALLBACKS == 1)
		#define DFSDM_FILTER_ERROR_INVALID_CALLBACK \
			((uint32_t)0x00000004U) /*!< Invalid callback error occurs */
	#endif
	/**
	 * @}
	 */
	...
	#define DFSDM_CONTINUOUS_CONV_OFF                              \
		((uint32_t)0x00000000U) /*!< Conversion are not continuous \
								 */
	...
	/** @addtogroup DFSDM_Exported_Functions_Group1_Channel Channel initialization and
	...
	HAL_StatusTypeDef
		HAL_DFSDM_Channel_RegisterCallback(DFSDM_Channel_HandleTypeDef* hdfsdm_channel,
	...
	HAL_StatusTypeDef
		HAL_DFSDM_ChannelPollForCkab(const DFSDM_Channel_HandleTypeDef* hdfsdm_channel,
									 uint32_t Timeout);
	...
	HAL_StatusTypeDef
		HAL_DFSDM_Filter_RegisterCallback(DFSDM_Filter_HandleTypeDef* hdfsdm_filter,
	...
	HAL_StatusTypeDef
		HAL_DFSDM_Filter_UnRegisterCallback(DFSDM_Filter_HandleTypeDef* hdfsdm_filter,
											HAL_DFSDM_Filter_CallbackIDTypeDef CallbackID);
	HAL_StatusTypeDef
		HAL_DFSDM_Filter_RegisterAwdCallback(DFSDM_Filter_HandleTypeDef* hdfsdm_filter,
	...
	HAL_StatusTypeDef HAL_DFSDM_FilterInjectedStart_DMA(DFSDM_Filter_HandleTypeDef* hdfsdm_filter,
														int32_t* pData, uint32_t Length);
	HAL_StatusTypeDef
		HAL_DFSDM_FilterInjectedMsbStart_DMA(DFSDM_Filter_HandleTypeDef* hdfsdm_filter,
											 int16_t* pData, uint32_t Length);
	...
	HAL_StatusTypeDef
		HAL_DFSDM_FilterPollForRegConversion(DFSDM_Filter_HandleTypeDef* hdfsdm_filter,
											 uint32_t Timeout);
	HAL_StatusTypeDef
		HAL_DFSDM_FilterPollForInjConversion(DFSDM_Filter_HandleTypeDef* hdfsdm_filter,
											 uint32_t Timeout);
