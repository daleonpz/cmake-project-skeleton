...
#include "stm32f7xx.h"
	 ...
	 *       VBAT modes) is enabled. If BRE is reset, the backup Regulator is switched off. The
	 * backup SRAM can still be used but its content will be lost in the Standby and VBAT modes.
	 * Once set, the application must wait that the Backup Regulator Ready flag (BRR) is set to
	 * indicate that the data written into the RAM will be maintained in the Standby and VBAT modes.
	 * @retval None
	 */
	...
	{
		return (uint32_t)(READ_BIT(PWR->CR1, (PWR_CR1_PDDS | PWR_CR1_LPDS | PWR_CR1_FPDS |
											  PWR_CR1_LPUDS | PWR_CR1_MRUDS)));
	}
	...
	/**
	 * @brief  Indicate whether the Regulator is ready in the selected voltage range or if its
	 * output voltage is still changing to the required voltage level
