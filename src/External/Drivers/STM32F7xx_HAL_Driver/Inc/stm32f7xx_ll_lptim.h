...
#include "stm32f7xx.h"
	...
	#define LL_LPTIM_OUTPUT_POLARITY_INVERSE                                                       \
		LPTIM_CFGR_WAVPOL /*!<The LPTIM output reflects the inverse of the compare results between \
	...
	#define LL_LPTIM_PRESCALER_DIV16                               \
		LPTIM_CFGR_PRESC_2 /*!<Prescaler division factor is set to \
							  16*/
	...
	#define LL_LPTIM_PRESCALER_DIV128                            \
		LPTIM_CFGR_PRESC /*!<Prescaler division factor is set to \
							128*/
	/**
	 * @}
	 */
	...
	#define LL_LPTIM_TRIG_SOURCE_COMP1 \
		(LPTIM_CFGR_TRIGSEL_2 |        \
		 LPTIM_CFGR_TRIGSEL_1) /*!<External input trigger is connected to COMP1 output*/
	#define LL_LPTIM_TRIG_SOURCE_COMP2 \
		LPTIM_CFGR_TRIGSEL /*!<External input trigger is connected to COMP2 output*/
	/**
	 * @}
	 */
	 ...
	 * @retval None
	 */
	__STATIC_INLINE void LL_LPTIM_ConfigTrigger(LPTIM_TypeDef* LPTIMx, uint32_t Source,
												uint32_t Filter, uint32_t Polarity)
	{
	...
	/**
	  * @brief  Informs application whether the APB bus write operation to the LPTIMx_CMP register
	...
	/**
	  * @brief  Informs application whether the APB bus write operation to the LPTIMx_ARR register
	  has been successfully completed. If so, a new one can be initiated.
	...
	/**
	  * @brief  Informs the application whether the counter direction has changed from down to up
	  (when the LPTIM instance operates in encoder mode).
	...
	/**
	  * @brief  Informs the application whether the counter direction has changed from up to down
	  (when the LPTIM instance operates in encoder mode).
	...
	/**
	 * @brief  Indicates whether the compare register write completed interrupt (CMPOKIE) is
	 * enabled.
	...
	/**
	 * @brief  Indicates whether the autoreload register write completed interrupt (ARROKIE) is
	 * enabled.
