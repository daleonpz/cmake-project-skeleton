...
#include "stm32f7xx.h"
	...
	typedef struct
	{
		...
		uint32_t
			LastBitClockPulse; /*!< Specifies whether the clock pulse corresponding to the last
	 ...
	 *         and current operations are discarded. The configuration of the USART is kept, but all
	 * the status flags, in the USARTx_ISR are set to their default values.
	 ...
	 * @note   When this function is enabled, USART is able to wake up the MCU from Stop mode,
	 * provided that USART clock selection is HSI or LSE in RCC.
	 ...
	 * @note   This function selects if hardware parity control (generation and detection) is
	 * enabled or disabled. When the parity control is enabled (Odd or Even), computed parity bit is
	 * inserted at the MSB position (9th or 8th bit depending on data width) and parity is checked
	...
	/**
	 * @brief  Configure Clock signal format (Phase Polarity and choice about output of last bit
	 * clock pulse)
	 * @note   Macro IS_USART_INSTANCE(USARTx) can be used to check whether or not
	 *         Synchronous mode is supported by the USARTx instance.
	 ...
	 * @note   MSB First means data is transmitted/received with the MSB first, following the start
	 * bit. LSB First means data is transmitted/received with data bit 0 first, following the start
	 * bit.
	 ...
	 * @note   Macro IS_USART_AUTOBAUDRATE_DETECTION_INSTANCE(USARTx) can be used to check whether
	 ...
	 *         (Baud rate value != 0)
	 * @note   In case of oversampling by 16 and 8, BRR content must be greater than or equal to
	 * 16d.
	 ...
	 * mode. In transmission mode, it specifies the number of automatic retransmission retries,
	 * before generating a transmission error (FE bit set). In reception mode, it specifies the
	 * number or erroneous reception trials, before generating a reception error (RXNE and PE bits
	 * set)
	...
	/** @defgroup USART_LL_EF_Configuration_HalfDuplex Configuration functions related to Half
	 * Duplex feature
	 * @{
	 */
	...
	/** @defgroup USART_LL_EF_Configuration_DE Configuration functions related to Driver Enable
	 * feature
	 * @{
	 */

	/**
	 * @brief  Set DEDT (Driver Enable De-Assertion Time), Time value expressed on 5 bits ([4:0]
	 * bits).
	 ...
	 * framing error, overrun error or noise flag (FE=1 or ORE=1 or NF=1 in the USARTx_ISR
	 * register). 0: Interrupt is inhibited 1: An interrupt is generated when FE=1 or ORE=1 or NF=1
	...
	/* Function available only on devices supporting Transmit Complete before Guard Time feature */
	/**
	 * @brief  Check if the Smartcard Transmission Complete Before Guard Time Interrupt is enabled
	 * or disabled.
	 * @note   Macro IS_SMARTCARD_INSTANCE(USARTx) can be used to check whether or not
	 *         Smartcard feature is supported by the USARTx instance.
