...
#include "stm32f7xx.h"
	...
	{
		uint32_t
			TransferDirection; /*!< Specifies the SPI unidirectional or bidirectional data mode.
		...
		uint32_t NSS; /*!< Specifies whether the NSS signal is managed by hardware (NSS pin) or by
						 software using the SSI bit. This parameter can be a value of @ref
						 SPI_LL_EC_NSS_MODE.
	 ...
	 * @note   This bit should not be changed when communication is ongoing. This bit is not used in
	 * SPI TI mode.
	 ...
	 * @note   This bit has to be written as soon as the last data is written in the SPIx_DR
	 * register.
	 ...
	 * @note   This bit controls the generation of an interrupt when an error condition occurs
	 * (CRCERR, OVR, MODF in SPI mode, FRE at TI mode).
	...
	{
		return ((READ_BIT(SPIx->I2SCFGR, SPI_I2SCFGR_ASTRTEN) == (SPI_I2SCFGR_ASTRTEN)) ? 1UL :
																						  0UL);
	}
	 ...
	 * @note   This bit controls the generation of an interrupt when an error condition occurs (OVR,
	 * UDR and FRE in I2S mode).
