...
#include "stm32f7xx_hal_def.h"
	...
	/** @defgroup JPEG_Configuration_Structure_definition JPEG Configuration for encoding Structure
	 * definition
	...
	typedef struct
	{
		...
		uint32_t ChromaSubsampling; /*!< Chroma Subsampling in case of YCBCR or CMYK color space,
									   0-> 4:4:4 , 1-> 4:2:2, 2 -> 4:1:1, 3 -> 4:2:0 This parameter
									   can be a value of @ref JPEG_ChromaSubsampling */
	...
	#endif /* (USE_HAL_JPEG_REGISTER_CALLBACKS) */
	{
		...
		uint8_t*
			pJpegInBuffPtr; /*!< Pointer to JPEG processing (encoding, decoding,...) input buffer */

		uint8_t* pJpegOutBuffPtr; /*!< Pointer to JPEG processing (encoding, decoding,...) output
									 buffer */
		...
		uint8_t CustomQuanTable; /*!< If set to 1 specify that user customized quantization tables
									are used */
		...
		void (*EncodeCpltCallback)(
			struct __JPEG_HandleTypeDef* hjpeg); /*!< JPEG Encode complete callback */
		void (*DecodeCpltCallback)(
			struct __JPEG_HandleTypeDef* hjpeg); /*!< JPEG Decode complete callback */
		...
		void (*MspDeInitCallback)(
			struct __JPEG_HandleTypeDef* hjpeg); /*!< JPEG Msp DeInit callback  */
	...
	#if(USE_HAL_JPEG_REGISTER_CALLBACKS == 1)
	/** @defgroup HAL_JPEG_Callback_ID_enumeration_definition HAL JPEG Callback ID enumeration
	 * definition
	 ...
	 * @{
	 */
	typedef void (*pJPEG_CallbackTypeDef)(
		JPEG_HandleTypeDef* hjpeg); /*!< pointer to a common JPEG callback function */
	typedef void (*pJPEG_InfoReadyCallbackTypeDef)(
		JPEG_HandleTypeDef* hjpeg,
		JPEG_ConfTypeDef* pInfo); /*!< pointer to an Info ready JPEG callback function */
	typedef void (*pJPEG_GetDataCallbackTypeDef)(
		JPEG_HandleTypeDef* hjpeg,
		uint32_t NbDecodedData); /*!< pointer to a Get data JPEG callback function */
	typedef void (*pJPEG_DataReadyCallbackTypeDef)(
		JPEG_HandleTypeDef* hjpeg, uint8_t* pDataOut,
	...
	#define HAL_JPEG_ERROR_HUFF_TABLE \
		((uint32_t)0x00000001U) /*!< HUffman Table programming error */
	#define HAL_JPEG_ERROR_QUANT_TABLE \
		((uint32_t)0x00000002U) /*!< Quantization Table programming error */
	...
	#if(USE_HAL_JPEG_REGISTER_CALLBACKS == 1)
		#define HAL_JPEG_ERROR_INVALID_CALLBACK \
	 ...
	 * @{
	 */
	#define JPEG_FLAG_IFTF \
		((uint32_t)JPEG_SR_IFTF) /*!< Input FIFO is not full and is bellow its threshold flag */
	#define JPEG_FLAG_IFNFF \
		((uint32_t)JPEG_SR_IFNFF) /*!< Input FIFO Not Full Flag, a data can be written */
	#define JPEG_FLAG_OFTF \
		((uint32_t)JPEG_SR_OFTF) /*!< Output FIFO is not empty and has reach its threshold */
	#define JPEG_FLAG_OFNEF \
		((uint32_t)JPEG_SR_OFNEF) /*!< Output FIFO is not empty, a data is available  */
	#define JPEG_FLAG_EOCF                                                                      \
		((uint32_t)JPEG_SR_EOCF) /*!< JPEG Codec core has finished the encoding or the decoding \
									process and than last data has been sent to the output FIFO */
	#define JPEG_FLAG_HPDF                                                                       \
		((uint32_t)JPEG_SR_HPDF) /*!< JPEG Codec has finished the parsing of the headers and the \
	...
	#define JPEG_PAUSE_RESUME_INPUT_OUTPUT \
		((uint32_t)0x00000003U) /*!< Pause/Resume Input and Output FIFO Xfer*/
	/**
	 * @}
	 */
	...
	#define __HAL_JPEG_DISABLE(__HANDLE__) ((__HANDLE__)->Instance->CR &= ~JPEG_CR_JCEN)
		 ...
		 *         @arg JPEG_FLAG_IFTF  : The input FIFO is not full and is bellow its threshold
		 * flag
		 ...
		 *         @arg JPEG_FLAG_EOCF  : JPEG Codec core has finished the encoding or the decoding
		 * process and than last data has been sent to the output FIFO
	...
	#define __HAL_JPEG_CLEAR_FLAG(__HANDLE__, __FLAG__) \
		(((__HANDLE__)->Instance->CFR |= ((__FLAG__) & (JPEG_FLAG_EOCF | JPEG_FLAG_HPDF))))
	 ...
	 * @retval No return
	 */
	#define __HAL_JPEG_ENABLE_IT(__HANDLE__, __INTERRUPT__) \
	 ...
	 *         @arg JPEG_IT_HPD   : Header Parsing Done Interrupt
	 *
	 * @note    To disable an IT we must use MODIFY_REG macro to avoid writing "1" to the FIFO flush
	 * bits located in the same IT enable register (CR register).
	 * @retval  No return
	 */
	#define __HAL_JPEG_DISABLE_IT(__HANDLE__, __INTERRUPT__) \
		MODIFY_REG((__HANDLE__)->Instance->CR, (__INTERRUPT__), 0UL)
	 ...
	 */
	#define __HAL_JPEG_GET_IT_SOURCE(__HANDLE__, __INTERRUPT__) \
		((__HANDLE__)->Instance->CR & (__INTERRUPT__))
	...
	HAL_StatusTypeDef HAL_JPEG_SetUserQuantTables(JPEG_HandleTypeDef* hjpeg, uint8_t* QTable0,
												  uint8_t* QTable1, uint8_t* QTable2,
												  uint8_t* QTable3);
	...
	#define IS_JPEG_CHROMASUBSAMPLING(SUBSAMPLING)                                             \
		(((SUBSAMPLING) == JPEG_444_SUBSAMPLING) || ((SUBSAMPLING) == JPEG_420_SUBSAMPLING) || \
		 ((SUBSAMPLING) == JPEG_422_SUBSAMPLING))

	#define IS_JPEG_IMAGE_QUALITY(NUMBER) \
		(((NUMBER) >= JPEG_IMAGE_QUALITY_MIN) && ((NUMBER) <= JPEG_IMAGE_QUALITY_MAX))

	#define IS_JPEG_COLORSPACE(COLORSPACE)                                                         \
		(((COLORSPACE) == JPEG_GRAYSCALE_COLORSPACE) || ((COLORSPACE) == JPEG_YCBCR_COLORSPACE) || \
		 ((COLORSPACE) == JPEG_CMYK_COLORSPACE))

	#define IS_JPEG_PAUSE_RESUME_STATE(VALUE)                                             \
		(((VALUE) == JPEG_PAUSE_RESUME_INPUT) || ((VALUE) == JPEG_PAUSE_RESUME_OUTPUT) || \
