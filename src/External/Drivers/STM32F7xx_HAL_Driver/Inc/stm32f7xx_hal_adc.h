...
#include "stm32f7xx_hal_def.h"
	 ...
	 * @note   The setting of these parameters with function HAL_ADC_Init() is conditioned to ADC
	 ...
	 *          - For parameters 'ExternalTrigConv' and 'ExternalTrigConvEdge': ADC enabled, even
	 * with conversion on going. If ADC is not in the appropriate state to modify some parameters,
	 * these parameters setting is bypassed without error reporting (as it can be the expected
	 * behaviour in case of intended action to update another parameter (which fulfills the ADC
	...
	typedef struct
	{
		...
		uint32_t DataAlign; /*!< Specifies ADC data alignment to right (MSB on register bit 11 and
							   LSB on register bit 0) (default setting) or to left (if regular
							   group: MSB on register bit 15 and LSB on register bit 4, if injected
							   group (MSB kept as signed value due to potential negative value after
							   offset application): MSB on register bit 14 and LSB on register bit
							   3). This parameter can be a value of @ref ADC_Data_Align */
		uint32_t
			ScanConvMode; /*!< Configures the sequencer of regular and injected groups.
							 ...
							 rank). Scan direction is upward: from rank1 to rank 'n'. This parameter
							 can be a value of @ref ADC_Scan_mode. This parameter can be set to
							 ENABLE or DISABLE */
		uint32_t
			EOCSelection; /*!< Specifies what EOC (End Of Conversion) flag is used for conversion by
							 polling and interruption: end of conversion of each rank or complete
							 sequence. This parameter can be a value of @ref ADC_EOCSelection. Note:
							 For injected group, end of conversion (flag&IT) is raised only at the
							 end of the sequence. Therefore, if end of conversion is set to end of
							 each conversion, injected group should not be used with interruption
							 (HAL_ADCEx_InjectedStart_IT) or polling (HAL_ADCEx_InjectedStart and
							 HAL_ADCEx_InjectedPollForConversion). By the way, polling is still
							 possible since driver will use an estimated timing for end of injected
							 conversion. Note: If overrun feature is intended to be used, use ADC in
							 mode 'interruption' (function HAL_ADC_Start_IT() ) with parameter
							 EOCSelection set to end of each conversion or in mode 'transfer by DMA'
							 (function HAL_ADC_Start_DMA()). If overrun feature is intended to be
							 bypassed, use ADC in mode 'polling' or 'interruption' with parameter
							 EOCSelection must be set to end of sequence */
		uint32_t
			ContinuousConvMode; /*!< Specifies whether the conversion is performed in single mode
								   (one conversion) or continuous mode for regular group, after the
								   selected trigger occurred (software start or external trigger).
									 This parameter can be set to ENABLE or DISABLE. */
		uint32_t NbrOfConversion; /*!< Specifies the number of ranks that will be converted within
									 the regular group sequencer. To use regular group sequencer and
									 convert several ranks, parameter 'ScanConvMode' must be
									 enabled. This parameter must be a number between Min_Data = 1
									 and Max_Data = 16. */
		FunctionalState
			DiscontinuousConvMode; /*!< Specifies whether the conversions sequence of regular group
									  is performed in Complete-sequence/Discontinuous-sequence (main
									  sequence subdivided in successive parts). Discontinuous mode
									  is used only if sequencer is enabled (parameter
									  'ScanConvMode'). If sequencer is disabled, this parameter is
									  discarded. Discontinuous mode can be enabled only if
									  continuous mode is disabled. If continuous mode is enabled,
									  this parameter setting is discarded. This parameter can be set
									  to ENABLE or DISABLE. */
		uint32_t
			NbrOfDiscConversion; /*!< Specifies the number of discontinuous conversions in which the
									main sequence of regular group (parameter NbrOfConversion) will
									be subdivided. If parameter 'DiscontinuousConvMode' is disabled,
									this parameter is discarded. This parameter must be a number
									between Min_Data = 1 and Max_Data = 8. */
		uint32_t ExternalTrigConv; /*!< Selects the external event used to trigger the conversion
									  start of regular group. If set to ADC_SOFTWARE_START, external
									  triggers are disabled. If set to external trigger source,
									  triggering is on event rising edge by default. This parameter
									  can be a value of @ref ADC_External_trigger_Source_Regular */
		uint32_t ExternalTrigConvEdge; /*!< Selects the external trigger edge of regular group.
											If trigger is set to ADC_SOFTWARE_START, this parameter
										  is discarded. This parameter can be a value of @ref
										  ADC_External_trigger_edge_Regular */
		FunctionalState
			DMAContinuousRequests; /*!< Specifies whether the DMA requests are performed in one shot
									  mode (DMA transfer stop when number of conversions is reached)
										or in Continuous mode (DMA transfer unlimited, whatever
									  number of conversions). Note: In continuous mode, DMA must be
									  configured in circular mode. Otherwise an overrun will be
									  triggered when DMA buffer maximum pointer is reached. Note:
									  ...
									  launch a conversion). This parameter can be set to ENABLE or
									  DISABLE. */
	} ADC_InitTypeDef;

	/**
	 * @brief  Structure definition of ADC channel for regular group
	 * @note   The setting of these parameters with function HAL_ADC_ConfigChannel() is conditioned
	 * to ADC state. ADC can be either disabled or enabled without conversion on going on regular
	 * group.
	 */
	typedef struct
	{
		...
		uint32_t
			SamplingTime; /*!< Sampling time value to be set for the selected channel.
							 ...
							 groups. If this same channel has been previously configured in the
							 other group (regular/injected), it will be updated to last setting.
...
#define HAL_ADC_STATE_BUSY_INTERNAL \
	((uint32_t)0x00000002U) /*!< ADC is busy to internal process (initialization, calibration) */
...
#define HAL_ADC_STATE_REG_BUSY                                                                    \
	((uint32_t)0x00000100U) /*!< A conversion on group regular is ongoing or can occur (either by \
							   continuous mode, external trigger, low power auto power-on (if     \
							   feature available), multimode ADC master control (if feature       \
							   available)) */
#define HAL_ADC_STATE_REG_EOC \
	((uint32_t)0x00000200U) /*!< Conversion data available on group regular */
#define HAL_ADC_STATE_REG_OVR ((uint32_t)0x00000400U) /*!< Overrun occurrence */

/* States of ADC group injected */
#define HAL_ADC_STATE_INJ_BUSY                                                                     \
	((uint32_t)0x00001000U) /*!< A conversion on group injected is ongoing or can occur (either by \
							   auto-injection mode, external trigger, low power auto power-on (if  \
							   feature available), multimode ADC master control (if feature        \
							   available)) */
#define HAL_ADC_STATE_INJ_EOC \
	((uint32_t)0x00002000U) /*!< Conversion data available on group injected */

/* States of ADC analog watchdogs */
#define HAL_ADC_STATE_AWD1 \
	((uint32_t)0x00010000U) /*!< Out-of-window occurrence of analog watchdog 1 */
#define HAL_ADC_STATE_AWD2                                                                    \
	((uint32_t)0x00020000U) /*!< Not available on STM32F7 device: Out-of-window occurrence of \
							   analog watchdog 2 */
#define HAL_ADC_STATE_AWD3                                                                    \
	((uint32_t)0x00040000U) /*!< Not available on STM32F7 device: Out-of-window occurrence of \
							   analog watchdog 3 */

/* States of ADC multi-mode */
#define HAL_ADC_STATE_MULTIMODE_SLAVE                                                           \
	((uint32_t)0x00100000U) /*!< Not available on STM32F7 device: ADC in multimode slave state, \
...
#define HAL_ADC_ERROR_NONE \
	((uint32_t)0x00U) /*!< No error                                              */
#define HAL_ADC_ERROR_INTERNAL                                            \
	((uint32_t)0x01U) /*!< ADC IP internal error: if problem of clocking, \
						  enable/disable, erroneous state                       */
#define HAL_ADC_ERROR_OVR \
	((uint32_t)0x02U) /*!< Overrun error                                         */
#define HAL_ADC_ERROR_DMA \
	((uint32_t)0x04U) /*!< DMA transfer error                                    */
#if(USE_HAL_ADC_REGISTER_CALLBACKS == 1)
...
#define IS_ADC_EXT_TRIG(__REGTRIG__)                     \
	(((__REGTRIG__) == ADC_EXTERNALTRIGCONV_T1_CC1) ||   \
	 ...
	 ((__REGTRIG__) == ADC_EXTERNALTRIGCONV_EXT_IT11) || ((__REGTRIG__) == ADC_SOFTWARE_START))
...
#define IS_ADC_SAMPLE_TIME(__TIME__)                                                        \
	(((__TIME__) == ADC_SAMPLETIME_3CYCLES) || ((__TIME__) == ADC_SAMPLETIME_15CYCLES) ||   \
	 ((__TIME__) == ADC_SAMPLETIME_28CYCLES) || ((__TIME__) == ADC_SAMPLETIME_56CYCLES) ||  \
	 ((__TIME__) == ADC_SAMPLETIME_84CYCLES) || ((__TIME__) == ADC_SAMPLETIME_112CYCLES) || \
	 ((__TIME__) == ADC_SAMPLETIME_144CYCLES) || ((__TIME__) == ADC_SAMPLETIME_480CYCLES))
#define IS_ADC_EOCSelection(__EOCSelection__)                                                   \
	(((__EOCSelection__) == ADC_EOC_SINGLE_CONV) || ((__EOCSelection__) == ADC_EOC_SEQ_CONV) || \
	 ((__EOCSelection__) == ADC_EOC_SINGLE_SEQ_CONV))
#define IS_ADC_EVENT_TYPE(__EVENT__) \
	(((__EVENT__) == ADC_AWD_EVENT) || ((__EVENT__) == ADC_OVR_EVENT))
...
#define IS_ADC_REGULAR_RANK(__RANK__)                                              \
	(((__RANK__) == ADC_REGULAR_RANK_1) || ((__RANK__) == ADC_REGULAR_RANK_2) ||   \
	 ((__RANK__) == ADC_REGULAR_RANK_3) || ((__RANK__) == ADC_REGULAR_RANK_4) ||   \
	 ((__RANK__) == ADC_REGULAR_RANK_5) || ((__RANK__) == ADC_REGULAR_RANK_6) ||   \
	 ((__RANK__) == ADC_REGULAR_RANK_7) || ((__RANK__) == ADC_REGULAR_RANK_8) ||   \
	 ((__RANK__) == ADC_REGULAR_RANK_9) || ((__RANK__) == ADC_REGULAR_RANK_10) ||  \
	 ((__RANK__) == ADC_REGULAR_RANK_11) || ((__RANK__) == ADC_REGULAR_RANK_12) || \
	 ((__RANK__) == ADC_REGULAR_RANK_13) || ((__RANK__) == ADC_REGULAR_RANK_14) || \
	 ((__RANK__) == ADC_REGULAR_RANK_15) || ((__RANK__) == ADC_REGULAR_RANK_16))
...
#define IS_ADC_REGULAR_LENGTH(__LENGTH__) \
	(((__LENGTH__) >= ((uint32_t)1)) && ((__LENGTH__) <= ((uint32_t)16)))
#define IS_ADC_REGULAR_DISC_NUMBER(__NUMBER__) \
	(((__NUMBER__) >= ((uint32_t)1)) && ((__NUMBER__) <= ((uint32_t)8)))
...
#define ADC_CR1_DISCONTINUOUS(_NBR_DISCONTINUOUSCONV_) \
	(((_NBR_DISCONTINUOUSCONV_)-1) << ADC_CR1_DISCNUM_Pos)
