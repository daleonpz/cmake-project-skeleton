	...
	typedef struct
	{
		...
		uint32_t USERConfig; /*!< Program the FLASH User Option Byte: WWDG_SW / IWDG_SW / RST_STOP /
								RST_STDBY / IWDG_FREEZE_STOP / IWDG_FREEZE_SANDBY / nDBANK / nDBOOT.
								  nDBANK / nDBOOT are only available for STM32F76xxx/STM32F77xxx
								devices */
		...
		uint32_t
			PCROPRdp; /*!< Set the PCROP_RDP option.
 ...
 */
	...
	#define OB_NDBANK_DUAL_BANK                                           \
		((uint32_t)0x00000000U) /*!< NDBANK bit is reset : Dual Bank mode \
								 */
/**
 * @}
 */
