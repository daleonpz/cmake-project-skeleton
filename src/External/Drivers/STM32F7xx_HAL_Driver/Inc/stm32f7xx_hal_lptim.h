...
#include "stm32f7xx_hal_def.h"
	...
	{
		uint32_t
			Polarity; /*!< Selects the polarity of the active edge for the counter unit
		...
		uint32_t
			SampleTime; /*!< Selects the clock sampling time to configure the clock glitch filter.
						Note: This parameter is used only when Ultra low power clock source is used.
	...
	typedef struct
	{
		...
		uint32_t
			SampleTime; /*!< Selects the trigger sampling time to configure the clock glitch filter.
						Note: This parameter is used only when an external trigger is used.
	...
	#define __HAL_LPTIM_WAKEUPTIMER_EXTI_ENABLE_IT() \
		(EXTI->IMR |= LPTIM_EXTI_LINE_WAKEUPTIMER_EVENT)
	...
	#define __HAL_LPTIM_WAKEUPTIMER_EXTI_ENABLE_EVENT() \
		(EXTI->EMR |= LPTIM_EXTI_LINE_WAKEUPTIMER_EVENT)
	 ...
	 * @retval None.
	 */
	#define __HAL_LPTIM_WAKEUPTIMER_EXTI_ENABLE_FALLING_EDGE() \
	...
	/**
	 * @brief  Disable rising & falling edge trigger on the LPTIM Wake-up Timer associated Exti
	 * line.
	 * @retval None.
	 */
	...
	/**
	 * @brief Check whether the LPTIM Wake-up Timer associated Exti line interrupt flag is set or
	 * not.
	 * @retval Line Status.
	 */
	 ...
	 * @retval None.
	 */
	#define __HAL_LPTIM_WAKEUPTIMER_EXTI_GENERATE_SWIT() \
		(EXTI->SWIER |= LPTIM_EXTI_LINE_WAKEUPTIMER_EVENT)
	...
	HAL_StatusTypeDef HAL_LPTIM_PWM_Start(LPTIM_HandleTypeDef* hlptim, uint32_t Period,
										  uint32_t Pulse);
	...
	#define IS_LPTIM_AUTORELOAD(__AUTORELOAD__) \
		((0x00000001UL <= (__AUTORELOAD__)) && ((__AUTORELOAD__) <= 0x0000FFFFUL))
