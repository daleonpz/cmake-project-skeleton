...
#include "stm32f7xx.h"
	...
	typedef struct
	{
		...
		uint32_t
			OutputBlue; /*!< Specifies the Blue value of the output image.
							 - This parameter must be a number between:
							   Min_Data = 0x00 and Max_Data = 0xFF if ARGB8888 color mode is
						   selected.
							 - This parameter must be a number between:
							 ...
							 - This parameter must be a number between:
							   Min_Data = 0x00 and Max_Data = 0x1F if ARGB1555 color mode is
						   selected.
							 - This parameter must be a number between:
							   Min_Data = 0x00 and Max_Data = 0x0F if ARGB4444 color mode is
						   selected.
		...
		uint32_t OutputGreen; /*!< Specifies the Green value of the output image.
								 ...
								 selected.
								   - This parameter must be a number between:
									 Min_Data = 0x00 and Max_Data = 0xFF if RGB888 color mode is
								 selected.
								   - This parameter must be a number between:
									 Min_Data = 0x00 and Max_Data = 0x3F if RGB565 color mode is
								 selected.
								   - This parameter must be a number between:
									 Min_Data = 0x00 and Max_Data = 0x1F if ARGB1555 color mode is
								 selected.
		...
		uint32_t OutputAlpha; /*!< Specifies the Alpha channel of the output image.
								 ...
								 selected.
								   - This parameter must be a number between:
									 Min_Data = 0x00 and Max_Data = 0x01 if ARGB1555 color mode is
								 selected.
								   - This parameter must be a number between:
									 Min_Data = 0x00 and Max_Data = 0x0F if ARGB4444 color mode is
								 selected.
								   - This parameter is not considered if RGB888 or RGB565 color mode
								 is selected.
		...
		uint32_t
			NbrOfPixelsPerLines; /*!< Specifies the number of pixels per lines of the area to be
									transferred.
		...
		uint32_t
			RBSwapMode; /*!< Specifies the output Red Blue swap mode.
	...
	typedef struct
	{
		...
		uint32_t
			RBSwapMode; /*!< Specifies the foreground or background Red Blue swap mode.
							 This parameter can be one value of @ref DMA2D_LL_EC_RED_BLUE_SWAP .
	 ...
	 *         value based on the output color mode (ARGB8888, RGB888,  RGB565, ARGB1555 or
	 * ARGB4444) as set by @ref LL_DMA2D_SetOutputColorMode.
	...
	{
		return ((READ_BIT(DMA2Dx->FGPFCCR, DMA2D_FGPFCCR_START) == (DMA2D_FGPFCCR_START)) ? 1UL :
																							0UL);
	}
	...
	{
		return ((READ_BIT(DMA2Dx->BGPFCCR, DMA2D_BGPFCCR_START) == (DMA2D_BGPFCCR_START)) ? 1UL :
																							0UL);
	}
