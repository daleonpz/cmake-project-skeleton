...
#include "stm32f7xx.h"
	...
	typedef struct
	{
		...
		uint32_t
			TypeAcknowledge; /*!< Specifies the ACKnowledge or Non ACKnowledge condition after the
								address receive match code or next received byte. This parameter can
								be a value of @ref I2C_LL_EC_I2C_ACKNOWLEDGE.
...
(extended clock) enable bits */
	...
	/**
	  ...
	  * @param  DigitalFilter This parameter must be a value between Min_Data=0x00 (Digital filter
	  disabled) and Max_Data=0x0F (Digital filter enabled and filtering capability up to
	  15*ti2cclk).
	 ...
	 * @note   The macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
	 *         SMBus feature is supported by the I2Cx Instance.
	 * @note   This configuration can only be programmed when associated Timeout is disabled
	 * (TimeoutA and/orTimeoutB).
	...
	/**
	 * @brief  Configure the SMBus Clock TimeoutA (SCL low timeout or SCL and SDA high timeout
	 * depends on TimeoutA mode).
	 * @note   The macro IS_SMBUS_ALL_INSTANCE(I2Cx) can be used to check whether or not
	 *         SMBus feature is supported by the I2Cx Instance.
	 ...
	 * @note   This bit can be clear by software in order to flush the transmit data register
	 * (TXDR).
	 ...
	 * @note   Software end mode : TC flag is set when NBYTES data are transferre, stretching SCL
	 * low.
	 ...
	 * @note   The transfer is completed after the NBYTES data transfer(STOP or RESTART will
	 * follow).
	...
	/**
	  * @brief  Prepare the generation of a ACKnowledge or Non ACKnowledge condition after the
	  address receive match code or next received byte.
	...
	/**
	  * @brief  Enable automatic RESTART Read request condition for 10bit address header (master
	  mode).
	...
	/**
	 * @brief  Disable automatic RESTART Read request condition for 10bit address header (master
	 * mode).
	...
	/**
	 * @brief  Check if automatic RESTART Read request condition for 10bit address header is enabled
	 * or disabled.
	...
	/**
	 * @brief  Handles I2Cx communication when starting transfer or during transfer (TC or TCR flag
	 * are set).
