...
#include "stm32f7xx_hal_def.h"
	...
	typedef struct
	{
		...
		uint32_t NoDivider; /*!< Specifies whether master clock will be divided or not.
							   ...
							   can take any of the values without constraint since the input clock
							   of the audio block should be equal to the bit clock. There is no
							   MCLK_x clock which can be output. */

		uint32_t
			FIFOThreshold; /*!< Specifies SAI Block FIFO threshold.
		...
		uint32_t
			CompandingMode; /*!< Specifies the companding mode type.
		...
		uint32_t
			ClockStrobing; /*!< Specifies the SAI Block clock strobing edge sensitivity.
...
#define HAL_SAI_ERROR_CNREADY \
	((uint32_t)0x00000010U) /*!< codec not ready                             */
