...
#include "stm32f7xx_hal_def.h"
	...
	typedef struct
	{
		...
		uint32_t
			ClockMode; /* Specifies the Clock Mode. It indicates the level that clock takes between
	...
	#define QSPI_CS_HIGH_TIME_2_CYCLE \
		((uint32_t)                   \
			 QUADSPI_DCR_CSHT_0) /*!<nCS stay high for at least 2 clock cycles between commands*/
	#define QSPI_CS_HIGH_TIME_3_CYCLE \
		((uint32_t)                   \
			 QUADSPI_DCR_CSHT_1) /*!<nCS stay high for at least 3 clock cycles between commands*/
	...
	#define QSPI_CS_HIGH_TIME_5_CYCLE \
		((uint32_t)                   \
			 QUADSPI_DCR_CSHT_2) /*!<nCS stay high for at least 5 clock cycles between commands*/
	...
	#define QSPI_CS_HIGH_TIME_8_CYCLE \
		((uint32_t)                   \
			 QUADSPI_DCR_CSHT) /*!<nCS stay high for at least 8 clock cycles between commands*/
	/**
	 * @}
	 */
	...
	#define QSPI_ALTERNATE_BYTES_32_BITS                    \
		((uint32_t)QUADSPI_CCR_ABSIZE) /*!<32-bit alternate \
										  bytes*/
	/**
	 * @}
	 */
	...
	#define QSPI_INSTRUCTION_2_LINES                           \
		((uint32_t)QUADSPI_CCR_IMODE_1) /*!<Instruction on two \
										   lines*/
	...
	#define QSPI_DDR_HHC_HALF_CLK_DELAY \
		((uint32_t)                     \
			 QUADSPI_CCR_DHHC) /*!<Delay the data output by one half of system clock in DDR mode*/
	/**
	 * @}
	 */
	...
	#define QSPI_MATCH_MODE_OR                                        \
		((uint32_t)QUADSPI_CR_PMM) /*!<OR match mode between unmasked \
									  bits*/
	/**
	 * @}
	 */
	...
	#define QSPI_FLAG_FT                                                                       \
		QUADSPI_SR_FTF /*!<Fifo threshold flag: Fifo threshold reached or data left after read \
						  from memory is complete*/
	#define QSPI_FLAG_TC                                                                           \
		QUADSPI_SR_TCF /*!<Transfer complete flag: programmed number of data have been transferred \
						  or the transfer has been aborted*/
	#define QSPI_FLAG_TE                                                 \
		QUADSPI_SR_TEF /*!<Transfer error flag: invalid address is being \
						  accessed*/
	/**
	 * @}
	 */
