...
{ /* Make sure we have C-declarations in C++ programs */
	...
	/* Support for transparency. Switching it off makes Wm smaller and faster */
		...
		#define WM_SUPPORT_TRANSPARENCY                                                        \
			1 /* Should be defined outside of GUI_WINSUPPORT because of '#if GUI_WINSUPPORT && \
				 WM_SUPPORT_TRANSPARENCY' in some files */
	#endif
	...
	*/
	...
	typedef struct
	{
		 ...
		 */
		#define WM_CREATE \
			0x0001 /* The first message received, right after client has actually been created */
		#define WM_MOVE 0x0003 /* window has been moved (Same as WIN32) */

		#define WM_SIZE                                                                            \
			0x0005 /* Is sent to a window after its size has changed (Same as WIN32, do not change \
					  !) */

		#define WM_DELETE                                                                          \
			11 /* Delete (Destroy) command: This tells the client to free its data strutures since \
				  the window it is associates with no longer exists.*/
		...
		#define WM_GET_INSIDE_RECT \
			20 /* get inside rectangle: client rectangle minus pixels lost to effect */
		#define WM_GET_ID 21 /* Get id of widget */
		#define WM_SET_ID 22 /* Set id of widget */
		#define WM_GET_CLIENT_WINDOW \
			23 /* Get window handle of client window. Default is the same as window */
		...
		#define WM_NOTIFY_CHILD_HAS_FOCUS                          \
			32 /* Sent to parent when child receives / loses focus \
				*/

		#define WM_NOTIFY_OWNER_KEY \
			33 /* Some widgets (e.g. listbox) notify owner when receiving key messages */
		...
		#define WM_NOTIFY_PARENT \
			38 /* Notify parent. Information is detailed as notification code */
		#define WM_NOTIFY_PARENT_REFLECTION                                       \
			39 /* Notify parent reflection.                                       \
		...
		#define WM_GET_RADIOGROUP                                           \
			43 /* Send to all siblings and children of a radio control when \
				  selection changed */
		...
		#define WM_NOTIFICATION_CHILD_DELETED \
			7 /* Inform window that child is about to be deleted */
		 ...
		 * binary or operator.
		 */
		#define WM_CF_HASTRANS                                                                    \
			(1UL << 0) /* Has transparency. Needs to be defined for windows which do not fill the \
					  entire section of their (client) rectangle. */
		...
		#define WM_CF_DISABLED                                                 \
			(1UL << 4) /* Disabled: Does not receive PID (mouse & touch) input \
						*/

		/* Create only flags ... Not available as status flags */
		#define WM_CF_ACTIVATE \
			(1UL << 5) /* If automatic activation upon creation of window is desired */
		...
		#define WM_CF_ANCHOR_RIGHT                                                               \
			(1UL << 7) /* Right anchor ...  If parent is resized, distance to right  will remain \
						  const (left is default) */
		#define WM_CF_ANCHOR_BOTTOM                                                              \
			(1UL << 8) /* Bottom anchor ... If parent is resized, distance to bottom will remain \
						  const (top  is default) */
		#define WM_CF_ANCHOR_LEFT                                                                \
			(1UL << 9) /* Left anchor ...   If parent is resized, distance to left   will remain \
						  const (left is default) */
		#define WM_CF_ANCHOR_TOP                                                                  \
			(1UL << 10) /* Top anchor ...    If parent is resized, distance to top    will remain \
						   const (top  is default) */

		#define WM_CF_CONST_OUTLINE                                                               \
			(1UL << 11) /* Constant outline. This is relevant for transparent windows only. If a  \
						   window is transparent and does not have a constant outline, its        \
						   background is invalided instead of the window itself. This causes add. \
						   computation time when redrawing. */
		#define WM_CF_LATE_CLIP (1UL << 12)
		#define WM_CF_MEMDEV_ON_REDRAW (1UL << 13)

		#define WM_SF_INVALID_DRAW (1UL << 14)
		#define WM_SF_DELETE                                                                \
			(1UL << 15) /* Marks the window to be deleted within WM_Exec() when no callback \
						   routine is executed */
		...
		#define WM_CF_GESTURE \
			(1UL << 19) /* Marks the window to be a able to receive gesture messages */

		#define WM_CF_ZOOM \
			(1UL << 20) /* Window can be scaled automatically by multi touch gesture input */
	...
	typedef void (*WM_tfInvalidateParent)(const GUI_RECT* pInvalidRect, WM_HWIN hParent,
										  WM_HWIN hStop);
	...
	int WM_CheckScrollPos(WM_SCROLL_STATE* pScrollState, int Pos, int LowerDist,
						  int UpperDist); /* not to be documented (may change in future version) */
	void WM_ClrHasTrans(WM_HWIN hWin);
	WM_HWIN WM_CreateWindow(int x0, int y0, int xSize, int ySize, U32 Style, WM_CALLBACK* cb,
							int NumExtraBytes);
	WM_HWIN WM_CreateWindowAsChild(int x0, int y0, int xSize, int ySize, WM_HWIN hWinParent,
								   U32 Style, WM_CALLBACK* cb, int NumExtraBytes);
	...
	void WM_InvalidateWindowAndDescs(
		WM_HWIN hWin); /* not to be documented (may change in future version) */
	int WM_IsEnabled(WM_HWIN hObj);
	char WM_IsCompletelyCovered(
		WM_HWIN hWin); /* Checks if the window is completely covered by other windows */
	...
	int GUI_MEMDEV_BlurAndBlendWinBk(WM_HWIN hWin, int Period, U8 BlurDepth, U32 BlendColor,
									 U8 BlendIntens);
	...
	int WM_SetScrollbarH(WM_HWIN hWin,
						 int OnOff); /* not to be documented (may change in future version) */
	int WM_SetScrollbarV(WM_HWIN hWin,
						 int OnOff); /* not to be documented (may change in future version) */
	...
	int WM_CreateTimer(WM_HWIN hWin, int UserID, int Period,
					   int Mode); /* not to be documented (may change in future version) */
	void WM_DeleteTimer(WM_HWIN hWin,
						int UserId); /* not to be documented (may change in future version) */
		#else
	WM_HMEM WM_CreateTimer(WM_HWIN hWin, int UserID, int Period,
						   int Mode); /* not to be documented (may change in future version) */
	...
	void WM_CheckScrollBounds(
		WM_SCROLL_STATE* pScrollState); /* not to be documented (may change in future version) */
	...
	int WM_SetScrollValue(WM_SCROLL_STATE* pScrollState,
						  int v); /* not to be documented (may change in future version) */
	...
	void WM_GetInsideRectExScrollbar(
		WM_HWIN hWin, GUI_RECT* pRect); /* not to be documented (may change in future version) */
	...
	void WM_SendMessageNoPara(WM_HWIN hWin,
							  int MsgId); /* not to be documented (may change in future */
	...
	void WM_SetCaptureMove(WM_HWIN hWin, const GUI_PID_STATE* pState, int MinVisibility,
						   int LimitTop); /* Not yet documented */
