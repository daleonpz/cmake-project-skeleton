...
{ /* Make sure we have C-declarations in C++ programs */
	...
	typedef struct GUI_VNC_CONTEXT
	{
		...
		int LayerIndex;
		int BytesPerPixel;
		int BitsPerPixel; // Note, that from within the VNC server the function
						  // LCD_GetBitsBerPixel() can not be used because the VNC server runs in a
						  // separate thread and the device chain can change during the function
						  // call
		//
		...
		int (*pfStoreData)(struct GUI_VNC_CONTEXT* pContext, BUFFER_CB* pBCB, const U8* pData,
						   int NumBytes);
	...
	int GUI_VNC_Process(GUI_VNC_CONTEXT* pContext, GUI_tSend pfSend, GUI_tRecv pfReceive,
						void* pConnectInfo);
