...
{ // Make sure we have C-declarations in C++ programs
		 ...
		 *       States
		 */
		#define SPINBOX_STATE_PRESSED(x) \
			(U8)(                        \
				1 << (U8)                \
					x) // These flags are stored in (SPINBOX_OBJ->State)        | x must be 0 or 1
		#define SPINBOX_STATE_FOCUS \
			WIDGET_STATE_FOCUS // This is read from         (SPINBOX_OBJ->Widget.State)
	...
	typedef struct
	{
		GUI_COLOR aColorFrame[2]; // [0] Outer color of surrounding frame.         [1] Inner color
								  // of surrounding frame.
		GUI_COLOR aColorUpper[2]; // [0] Upper color of gradient for upper button. [1] Lower color
								  // of gradient for upper button.
		GUI_COLOR aColorLower[2]; // [0] Upper color of gradient for lower button. [1] Lower color
								  // of gradient for lower button.
		GUI_COLOR ColorArrow; // Color of the button arrow.
		GUI_COLOR ColorBk; // Color of the background.                      // See
						   // WIDGET_ITEM_CREATE in SPINBOX_DrawSkinFlex()
		GUI_COLOR ColorText; // Color of the text.                            // See
							 // WIDGET_ITEM_CREATE in SPINBOX_DrawSkinFlex()
	 ...
	 *       Creation
	 */
	SPINBOX_Handle SPINBOX_CreateEx(int x0, int y0, int xSize, int ySize, WM_HWIN hParent,
									int WinFlags, int Id, int Min, int Max);
	SPINBOX_Handle SPINBOX_CreateUser(int x0, int y0, int xSize, int ySize, WM_HWIN hParent,
									  int WinFlags, int Id, int Min, int Max, int NumExtraBytes);
	SPINBOX_Handle SPINBOX_CreateIndirect(const GUI_WIDGET_CREATE_INFO* pCreateInfo,
