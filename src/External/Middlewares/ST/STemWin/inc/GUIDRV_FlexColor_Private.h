...
{ /* Make sure we have C-declarations in C++ programs */
	...
	struct DRIVER_CONTEXT
	{
		...
		U16 RegEntryMode; // Can be used for storing additional configuration bits for 'EntryMode'
						  // register which is modified by the driver
		U16 Flags;
		...
		//
		// Functions for writing single items (data, cmd) regardless of the interface and getting
		// the status
		//
		...
		void (*pfReadRect_16bpp_B16)(GUI_DEVICE* _pDevice, int _x0, int _y0, int _x1, int _y1,
									 U16* _pBuffer);
		void (*pfReadRect_16bpp_B8)(GUI_DEVICE* _pDevice, int _x0, int _y0, int _x1, int _y1,
									U16* _pBuffer);
		void (*pfReadRect_18bpp_B9)(GUI_DEVICE* _pDevice, int _x0, int _y0, int _x1, int _y1,
									U32* _pBuffer);
		void (*pfReadRect_18bpp_B18)(GUI_DEVICE* _pDevice, int _x0, int _y0, int _x1, int _y1,
									 U32* _pBuffer);
		void (*pfReadRect_32bpp_B8)(GUI_DEVICE* _pDevice, int _x0, int _y0, int _x1, int _y1,
									U32* _pBuffer);
		//
		...
		//
		void (*pfDrawBitmap)(GUI_DEVICE* _pDevice, int _x0, int _y0, int _xsize, int _ysize,
							 int _BitsPerPixel, int _BytesPerLine, const U8* _pData, int _Diff,
							 const LCD_PIXELINDEX* _pTrans);
	...
	void GUIDRV_FlexColor__SetPixelIndexCache(GUI_DEVICE* pDevice, int x, int y,
											  LCD_PIXELINDEX PixelIndex);
	LCD_PIXELINDEX GUIDRV_FlexColor__GetPixelIndexNoCache(GUI_DEVICE* pDevice, int x, int y);
	void GUIDRV_FlexColor__SetPixelIndexNoCache(GUI_DEVICE* pDevice, int x, int y,
												LCD_PIXELINDEX PixelIndex);
	...
	void SIM_FlexColor_Config(GUI_DEVICE* pDevice, int Orientation, int xSize, int ySize,
							  int FirstSEG, int FirstCOM, int BitsPerPixel, int NumDummyReads);
