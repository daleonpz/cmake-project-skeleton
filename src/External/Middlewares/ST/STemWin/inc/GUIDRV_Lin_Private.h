...
{ /* Make sure we have C-declarations in C++ programs */
	...
	#define DEFAULT_CONTEXT_MEMBERS                                                           \
		...
		int Alpha;                                                                            \
		int IsVisible;                                                                        \
		void (*pfFillRect)(int /* LayerIndex */, int /* x0 */, int /* y0 */, int /* x1 */,    \
						   int /* y1 */, U32 /* PixelIndex */);                               \
		void (*pfCopyBuffer)(int /* LayerIndex */, int /* IndexSrc */, int /* IndexDst */);   \
		void (*pfDrawBMP1)(int /* LayerIndex */, int /* x */, int /* y */, U8 const* /* p */, \
						   int /* Diff */, int /* xSize */, int /* ySize */,                  \
						   int /* BytesPerLine */, const LCD_PIXELINDEX* /* pTrans */);       \
		void (*pfDrawBMP8)(int /* LayerIndex */, int /* x */, int /* y */, U8 const* /* p */, \
						   int /* xSize */, int /* ySize */, int /* BytesPerLine */,          \
						   const LCD_PIXELINDEX* /* pTrans */);                               \
		void (*pfCopyRect)(int /* LayerIndex */, int /* x0 */, int /* y0 */, int /* x1 */,    \
						   int /* y1 */, int /* xSize */, int /* ySize */);                   \
		void (*pfSetPos)(int /* LayerIndex */, int /* xPos */, int /* yPos */);
	...
	#define DEFAULT_MANAGEMENT_SETDEVFUNC()                                                       \
		case LCD_DEVFUNC_FILLRECT:                                                                \
			pContext->pfFillRect =                                                                \
				(void (*)(int LayerIndex, int x0, int y0, int x1, int y1, U32 PixelIndex))pFunc;  \
			break;                                                                                \
		case LCD_DEVFUNC_COPYBUFFER:                                                              \
		...
		case LCD_DEVFUNC_DRAWBMP_1BPP:                                                            \
			pContext->pfDrawBMP1 =                                                                \
				(void (*)(int LayerIndex, int x, int y, U8 const* p, int Diff, int xSize,         \
						  int ySize, int BytesPerLine, const LCD_PIXELINDEX* pTrans))pFunc;       \
			break;                                                                                \
		case LCD_DEVFUNC_DRAWBMP_8BPP:                                                            \
			pContext->pfDrawBMP8 =                                                                \
				(void (*)(int LayerIndex, int x, int y, U8 const* p, int xSize, int ySize,        \
						  int BytesPerLine, const LCD_PIXELINDEX* pTrans))pFunc;                  \
			break;                                                                                \
		case LCD_DEVFUNC_COPYRECT:                                                                \
			pContext->pfCopyRect = (void (*)(int LayerIndex, int x0, int y0, int x1, int y1,      \
											 int xSize, int ySize))pFunc;                         \
			break;                                                                                \
		case LCD_DEVFUNC_SETPOS:                                                                  \
	...
	#if(LCD_MIRROR == 2)
		#define MIRROR(x)                                               \
			x = ((x & 0x000000ffUL) << 8) | ((x & 0x0000ff00UL) >> 8) | \
				((x & 0x00ff0000UL) << 8) | ((x & 0xff000000UL) >> 8)
	#else
		#define MIRROR(x)                                                \
			x = ((x & 0x000000ffUL) << 24) | ((x & 0x0000ff00UL) << 8) | \
	 ...
	 *
	 * Purpose:
	 *   Sets the visibility of the given layer by sending a LCD_X_SETVIS command to
	 * LCD_X_DisplayDriver() (Requires special hardware support.)
	 */
	 ...
	 *
	 * Purpose:
	 *   Sets the position of the given layer by sending a LCD_X_SETPOS command to
	 * LCD_X_DisplayDriver() (Requires special hardware support.)
	 */
	static void _SetPos(GUI_DEVICE* pDevice, int xPos, int yPos)
	{
		...
		if(pDevice->u.pContext)
		{
			...
			else
			{
				...
				if((PosInfo.xLen <= 0) || (PosInfo.yLen <= 0) || (PosInfo.xPos >= xSizeDisplay) ||
				   (PosInfo.yPos >= ySizeDisplay))
				{
	 ...
	 *
	 * Purpose:
	 *   Sets the alpha value of the given layer by sending a LCD_X_SETALPHA command to
	 * LCD_X_DisplayDriver() (Requires special hardware support.)
	 */
	 ...
	 *
	 * Purpose:
	 *   Sets the alpha mode of the given layer by sending a LCD_X_SETALPHAMODE command to
	 * LCD_X_DisplayDriver() (Requires special hardware support.)
	 */
	 ...
	 *
	 * Purpose:
	 *   Sets the chroma mode of the given layer by sending a LCD_X_SETCHROMAMODE command to
	 * LCD_X_DisplayDriver() (Requires special hardware support.)
	 */
	 ...
	 *
	 * Purpose:
	 *   Sets the chroma values of the given layer by sending a LCD_X_SETCHROMA command to
	 * LCD_X_DisplayDriver() (Requires special hardware support.)
	 */
	...
	#if defined(WIN32)
		SIM_Lin_SetVRAMSize(pDevice->LayerIndex, pContext->vxSize, pContext->vySize,
							pContext->xSize, pContext->ySize);
	#endif
	}
