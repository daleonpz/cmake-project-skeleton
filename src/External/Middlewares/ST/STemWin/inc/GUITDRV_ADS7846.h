...
(2) The driver needs some configuration variables filled in to be able to calculate
	...
	- xLog1      : Logical pixel value of horizontal reference point 1. Typically horizontal screen
resolution -1.
	...
	- yLog1      : Logical pixel value of vertical reference point 1. Typically vertical screen
resolution -1.
...
{ /* Make sure we have C-declarations in C++ programs */
	...
	typedef struct
	{
		...
		U16(*pfGetResult)
		(void); // Retrieves the result of the AD conversion. 4 dummy bytes have to be shifted out
				// to the left.
		...
		//
		char (*pfGetPENIRQ)(
			void); // Retrieves the status of the PENIRQ line to detect a touch event.
		//
		// Optional, touch recognition via touch pressure measurement (4)
		//
		int PressureMin; // Minimum pressure threshold. A measured pressure below this value means
						 // we do not have a valid touch event.
		int PressureMax; // Maximum pressure threshold. A measured pressure above this value means
						 // we do not have a valid touch event.
		int PlateResistanceX; // Resistance of the X-plate of the touch screen. This value is needed
							  // for calculation of the touch pressure.
