 ...
 */
#define XY2PTR(x, y)                                        \
	(PIXEL*)(pContext->pData + y * pContext->BytesPerLine + \
			 x * pContext->pDrawingAPI->BytesPerPixel)
 ...
 *       Draw Bitmap 1 BPP
 */
static void _DrawBitLine1BPP(GUI_DEVICE* pDevice, unsigned x, unsigned y, U8 const* p, int Diff,
 ...
 *       Draw Bitmap 2 BPP
 */
static void _DrawBitLine2BPP(GUI_DEVICE* pDevice, int x, int y, U8 const* p, int Diff, int xsize,
 ...
 *       Draw Bitmap 4 BPP
 */
static void _DrawBitLine4BPP(GUI_DEVICE* pDevice, int x, int y, U8 const* p, int Diff, int xsize,
							 const LCD_PIXELINDEX* pTrans)
{
	LCD_PIXELINDEX Pixels, PixelIndex;
	int CurrentPixel, Shift, Index;
 ...
 *       Draw Bitmap 8 BPP
 */
static void _DrawBitLine8BPP(GUI_DEVICE* pDevice, int x, int y, U8 const* p, int xsize,
							 const LCD_PIXELINDEX* pTrans)
{
 ...
 *       _DrawBitmap_CX
 */
static void _DrawBitmap_CX(GUI_DEVICE* pDevice, int x0, int y0, int xSize, int ySize,
						   ...
						   const LCD_PIXELINDEX* pTrans)
{
	...
	pDevice->pDeviceAPI->pfDrawBitmap(
		pDevice, x0_phys, y0_phys, x1_phys - x0_phys + 1, y1_phys - y0_phys + 1,
		pContext->pDrawingAPI->BytesPerPixel << 3,
		pContext->pDrawingAPI->BytesPerPixel * pContext->vxSize, (U8*)pDataBM, 0, NULL);
}
...
static void _FillRect_CX(GUI_DEVICE* pDevice, int x0, int y0, int x1, int y1)
{
	...
	pDevice->pDeviceAPI->pfDrawBitmap(
		pDevice, x0_phys, y0_phys, x1_phys - x0_phys + 1, y1_phys - y0_phys + 1,
		pContext->pDrawingAPI->BytesPerPixel << 3,
		pContext->pDrawingAPI->BytesPerPixel * pContext->vxSize, (U8*)pData, 0, NULL);
}
 ...
 */
const GUI_ORIENTATION_API API_NAME = {_DrawBitmap_CX, _DrawHLine_CX,	 _DrawVLine_CX,
									  _FillRect_CX,	  _GetPixelIndex_CX, _SetPixelIndex_CX,
									  _XorPixel_CX,	  BYTES_PER_PIXEL};
