...
{ // Make sure we have C-declarations in C++ programs
		...
		#define MENU_CF_CLOSE_ON_SECOND_CLICK \
			(1 << 2) // Normally a menu closes only when clicked outside it
		#define MENU_CF_HIDE_DISABLED_SEL \
			(1 << 3) // Hides the selection when a disabled item is selected
		...
		#define MENU_IS_MENU \
			5 // Internal message of menu widget. Owner must call
	 ...
	 **********************************************************************
	 */
	MENU_Handle MENU_CreateIndirect(const GUI_WIDGET_CREATE_INFO* pCreateInfo, WM_HWIN hWinParent,
									int x0, int y0, WM_CALLBACK* cb);
	MENU_Handle MENU_CreateEx(int x0, int y0, int xSize, int ySize, WM_HWIN hParent, int WinFlags,
							  int ExFlags, int Id);
	MENU_Handle MENU_CreateUser(int x0, int y0, int xSize, int ySize, WM_HWIN hParent, int WinFlags,
								int ExFlags, int Id, int NumExtraBytes);
	...
	void MENU_Attach(MENU_Handle hObj, WM_HWIN hDestWin, int x, int y, int xSize, int ySize,
					 int Flags);
	...
	void MENU_Popup(MENU_Handle hObj, WM_HWIN hDestWin, int x, int y, int xSize, int ySize,
					int Flags);
