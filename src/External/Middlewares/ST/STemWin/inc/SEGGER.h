...
{ /* Make sure we have C-declarations in C++ programs */
	...
	typedef struct
	{
		...
		void (*pfDMB)(void); // Optional DMB function for Data Memory Barrier to make sure all
							 // memory operations are completed.
		void (*pfClean)(void* p, unsigned NumBytes); // Optional clean function for cached memory.
		void (*pfInvalidate)(void* p,
							 unsigned NumBytes); // Optional invalidate function for cached memory.
	} SEGGER_CACHE_CONFIG;
	...
	struct SEGGER_SNPRINTF_CONTEXT_struct
	{
		...
		void (*pfFlush)(
			SEGGER_SNPRINTF_CONTEXT*); // Callback executed once the buffer is full. Callback
									   // decides if the buffer gets cleared to store more or not.
	};

	typedef struct
	{
		void (*pfStoreChar)(SEGGER_BUFFER_DESC* pBufferDesc, SEGGER_SNPRINTF_CONTEXT* pContext,
							char c);
		int (*pfPrintUnsigned)(SEGGER_BUFFER_DESC* pBufferDesc, SEGGER_SNPRINTF_CONTEXT* pContext,
							   U32 v, unsigned Base, char Flags, int Width, int Precision);
		int (*pfPrintInt)(SEGGER_BUFFER_DESC* pBufferDesc, SEGGER_SNPRINTF_CONTEXT* pContext, I32 v,
	...
	typedef void (*SEGGER_pFormatter)(SEGGER_BUFFER_DESC* pBufferDesc,
									  ...
									  const SEGGER_PRINTF_API* pApi, va_list* pParamList, char Lead,
