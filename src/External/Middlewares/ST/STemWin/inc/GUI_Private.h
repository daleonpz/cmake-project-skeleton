...
{ /* Make sure we have C-declarations in C++ programs */
	...
	*/
	...
	GUI_MEMDEV_Handle GUI_MEMDEV__GetEmptyCopy32(GUI_MEMDEV_Handle hMem, int* pxSize, int* pySize,
												 int* pxPos, int* pyPos);
	...
	void GUI_AA__DrawCharAA2(int x0, int y0, int XSize, int YSize, int BytesPerLine,
							 const U8* pData);
	void GUI_AA__DrawCharAA4(int x0, int y0, int XSize, int YSize, int BytesPerLine,
							 const U8* pData);
	void GUI_AA__DrawCharAA8(int x0, int y0, int XSize, int YSize, int BytesPerLine,
							 const U8* pData);
	...
	U32* GUI__DoAlphaBlending(int x, int y, U32* pData, int xSize,
							  tLCDDEV_Index2Color* pfIndex2Color_DEV, int* pDone);
	...
	void GUI__CalcTextRect(const char* pText, const GUI_RECT* pTextRectIn, GUI_RECT* pTextRectOut,
						   int TextAlign);
	...
	extern U16 (*GUI__GetCursorPos_pfGetCharacter)(const char* s, int MaxNumChars, int Index,
												   int* pIsRTL);
	...
	LCD_PIXELINDEX* LCD_GetpPalConvTableBM(const LCD_LOGPALETTE* pLogPal, const GUI_BITMAP* pBitmap,
										   int LayerIndex);

	/* Setting a function for converting a color palette to an array of index values */
	void GUI_SetFuncGetpPalConvTable(LCD_PIXELINDEX* (*pFunc)(const LCD_LOGPALETTE* pLogPal,
	...
	void GUI__CreateBitmapFromStream(const GUI_BITMAP_STREAM* pBitmapHeader, const void* pData,
									 GUI_BITMAP* pBMP, GUI_LOGPALETTE* pPAL,
	...
	#define GUI__MEMCPY(pDest, pSrc, Cnt) \
		GUI__pfMemcpy((void*)(pDest), (const void*)(pSrc), (size_t)(Cnt))
	...
	*/
	...
	int GUI_GetBitmapPixelIndexEx(int BitsPerPixel, int BytesPerLine, const U8* pData, unsigned x,
								  unsigned y);

	void GUI__DrawBitmap16bpp(int x0, int y0, int xsize, int ysize, const U8* pPixel,
							  const LCD_LOGPALETTE* pLogPal, int xMag, int yMag,
							  tLCDDEV_Index2Color* pfIndex2Color,
							  const LCD_API_COLOR_CONV* pColorConvAPI);
	void GUI__DrawBitmapA16bpp(int x0, int y0, int xSize, int ySize, const U8* pPixel,
							   const LCD_LOGPALETTE* pLogPal, int xMag, int yMag,
							   tLCDDEV_Index2Color* pfIndex2Color);
	...
	void GUI__MixColorsBulk(U32* pFG, U32* pBG, U32* pDst, unsigned OffFG, unsigned OffBG,
							unsigned OffDest, unsigned xSize, unsigned ySize, U8 Intens);
	...
	#define LCDDEV_L0_Color2Index \
		GUI__apDevice[GUI_pContext->SelLayer]->pColorConvAPI->pfColor2Index
	#define LCDDEV_L0_Index2Color \
		GUI__apDevice[GUI_pContext->SelLayer]->pColorConvAPI->pfIndex2Color
	...
	#define LCDDEV_L0_GetPixelIndex \
		GUI__apDevice[GUI_pContext->SelLayer]->pDeviceAPI->pfGetPixelIndex
	#define LCDDEV_L0_SetPixelIndex \
		GUI__apDevice[GUI_pContext->SelLayer]->pDeviceAPI->pfSetPixelIndex
	...
	void GUI_ReadRectEx(int x0, int y0, int x1, int y1, LCD_PIXELINDEX* pBuffer,
						GUI_DEVICE* pDevice);

	void LCD_ReadRectNoClip(int x0, int y0, int x1, int y1, LCD_PIXELINDEX* pBuffer,
							GUI_DEVICE* pDevice);
	...
	/* The following 2 defines are only required for compatibility to older versions of the TTF
	 * library */
	...
	//
	extern LCD_PIXELINDEX* (*GUI_pfGetpPalConvTable)(const LCD_LOGPALETTE* pLogPal,
													 const GUI_BITMAP* pBitmap, int LayerIndex);
	...
	//
	extern void (*GUI__pfMixColorsBulk)(U32* pFG, U32* pBG, U32* pDst, unsigned OffFG,
	...
	GUI_EXTERN int
		GUI__BufferSize; // Required buffer size in pixels for alpha blending and/or antialiasing
