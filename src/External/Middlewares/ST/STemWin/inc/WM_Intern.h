...
{ /* Make sure we have C-declarations in C++ programs */
		...
		*/
		...
		#define WM_SF_DISABLED \
			WM_CF_DISABLED /* Disabled: Does not receive PID (mouse & touch) input */
		...
		#define WM_SF_INVALID                                                                     \
			WM_CF_ACTIVATE /* We reuse this flag, as it is create only and Invalid is status only \
							*/
			...
			#define WM_ASSERT_NOT_IN_PAINT()                                         \
				{                                                                    \
					if(WM__PaintCallbackCnt)                                         \
						GUI_DEBUG_ERROROUT(                                          \
							"Function may not be called from within a paint event"); \
				}
		#else
			#define WM_ASSERT_NOT_IN_PAINT()
		#endif
	...
	void WM__InvalidateRect(const GUI_RECT* pInvalidRect, WM_HWIN hParent, WM_HWIN hStop,
							U16 Flags);
