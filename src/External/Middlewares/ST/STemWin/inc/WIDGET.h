...
{ /* Make sure we have C-declarations in C++ programs */
	...
	typedef struct
	{
		...
		#endif
		#if !defined(WIDGET_USE_SCHEME_SMALL) && !defined(WIDGET_USE_SCHEME_MEDIUM) && \
			!defined(WIDGET_USE_SCHEME_LARGE)
		...
		#define WIDGET_ITEM_APPLY_PROPS \
			29 // Not to be documented. Use this message identifier to update the
	...
	void WIDGET__SetScrollState(WM_HWIN hWin, const WM_SCROLL_STATE* pVState,
								const WM_SCROLL_STATE* pState);
	void WIDGET__FillStringInRect(const char* pText, const GUI_RECT* pFillRect,
								  const GUI_RECT* pTextRectMax, const GUI_RECT* pTextRectAct);

	//
	// Function pointers for drawing streamed bitmaps
	//
	extern void (*GUI__pfDrawStreamedBitmap)(const void* p, int x, int y);
	extern int (*GUI__pfDrawStreamedBitmapEx)(GUI_GET_DATA_FUNC* pfGetData, const void* p, int x,
											  int y);
