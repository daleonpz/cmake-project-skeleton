...
{ // Make sure we have C-declarations in C++ programs
		...
		#define MULTIPAGE_SKIN_FRAME_ALL                                                         \
			(MULTIPAGE_SKIN_FRAME_LEFT | MULTIPAGE_SKIN_FRAME_RIGHT | MULTIPAGE_SKIN_FRAME_TOP | \
			 MULTIPAGE_SKIN_FRAME_BOTTOM)
	...
	typedef struct
	{
		U8 SelSideBorderInc; // Number of pixels to add on both sides when drawing the selected
							 // item.
	...
	typedef struct
	{
		...
		int Sel;
		U16 State;
		U8 FrameFlags; // Flags to let the drawing function know which parts of the frame to
					   // display.
		U8 PageStatus;
	 ...
	 **********************************************************************
	 */
	MULTIPAGE_Handle MULTIPAGE_Create(int x0, int y0, int xSize, int ySize, WM_HWIN hParent, int Id,
	...
	int MULTIPAGE_SetBitmapEx(MULTIPAGE_Handle hObj, const GUI_BITMAP* pBitmap, int x, int y,
							  int Index, int State);
